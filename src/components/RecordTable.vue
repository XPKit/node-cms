...
export default {
  ...
  methods: {
    ...
    async updateRecord(record) {
      const uploadObject = {}
      const oldRecord = _.find(this.recordList, { _id: record._id })
      ...
      const removeAttachments = _.filter(
        oldRecord && oldRecord._attachments,
        (item) => !_.find(record._attachments, { _id: item._id }),
      )
    ...
    async updateRecords() {
      try {
        ...
        const removedRecordList = _.filter(this.recordList, (item) => !_.find(this.clonedRecordList, { _id: item._id }))
