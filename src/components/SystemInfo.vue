...
const _showLogoutButton = computed(() => !_.get(window, 'disableJwtLogin', false))

function onGetRecordEdition(editing) {
  isEditing.value = editing
}

function _getNodeCmsVersion() {
  ...
  timer.value = setTimeout(() => {
    ...
    eventSource.value.onerror = (error) => {
      ...
      if (reconnectAttempts.value < maxReconnectAttempts) {
        ...
        const reconnectDelay = Math.min(1000 * 2 ** reconnectAttempts.value, 30000)
        console.log(
          `Attempting to reconnect system info stream in ${reconnectDelay}ms (attempt ${reconnectAttempts.value})`,
        )
...
async function _logout() {
  if (isEditing.value) {
    return window.DialogService.show({ event: 'logout', callback: () => _logout() })
  }
  await LoginService.logout()
}

function _timeAgo(current) {
  return Dayjs().subtract(parseInt(current, 10), 'second').fromNow()
}

function _convertBytes(megaBytes) {
  ...
  return (megaBytes / 1024 ** i).toFixed(1) + ' ' + sizes[i]
}
