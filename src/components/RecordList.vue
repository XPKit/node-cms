...
export default {
  ...
  computed: {
    ...
    filteredList() {
      ...
      _.each(this.list, (item) => {
        item._searchable = { id: false, keyFields: false, query: false }
      })
      if (this.sift.isQuery) {
        return _.each(this.list.filter(sift(this.query)), (item) => {
      ...
      let filteredRecords = _.filter(this.list, (item) => {
        ...
        for (const queryKey in this.query) {
          ...
          if (
            (_.isUndefined(value) === false && _.isArray(value) && _.includes(value, queryValue)) ||
            (!_.isArray(value) && value === queryValue)
          ) {
            qValues = qValues + 1
          }
        }
  ...
  methods: {
    ...
    allRecordsSelected() {
      ...
      return (
        _.get(
          _.filter(this.filteredList, (record) => !_.includes(ids, record._id)),
          'length',
          0,
        ) === 0
      )
    ...
    hasEditableRecords() {
      return (
        _.get(
          _.filter(this.filteredList, (item) => _.get(item, '_local', false)),
          'length',
          0,
        ) !== 0
      )
    ...
    isItemSelected(item) {
      return (
        (this.multiselect && _.includes(_.map(this.localMultiselectItems, '_id'), item._id)) ||
