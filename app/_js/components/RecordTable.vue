  ...
  <div class="record-table">
    ...
    <template v-if="!record">
      ...
      <ul v-show="resource.locales" class="locales">
        <li v-for="item in resource.locales" :key="item"
      ...
      <vue-table-generator v-if="isReady" :options="options"
                           :resource="resource" :schema="schema" :items="filteredList" :locale.sync="locale"
...
export default {
  ...
  methods: {
    ...
    async updateRecord (record) {
      ...
      _.each(this.resource.schema, (field, id) => {
        if (field.input === 'file') {
          return
        }
