...
describe('Driver Promise', () => {
  ...
  it('#invoke', async () => {
    ...
    driver.define('summ', ['a', 'b'], context => context.result(context.params.a + context.params.b))
    const math = driver.build()
    let result = await math.summ(2, 2)
  ...
  it('#invoke with optional params', async () => {
    ...
    const math = driver.build()
    let result = await math.power(2, 3)
    assert.equal(result, 8)
    result = await math.power(2)
    assert.equal(result, 4)
  })
  ...
  it('#beforeAll', async () => {
    ...
    const math = driver.build()
    let result = await math.summInc(2, 7)
    assert(trigger)
  ...
  it('#before', async () => {
    ...
    driver.define('summInc', ['a', 'b'], context => context.result(context.params.a + context.params.b))
    const math = driver.build()
    let result = await math.summ(2, 7)
    assert(!trigger)
    assert.equal(result, 9)
    result = await math.summInc(2, 7)
    assert(trigger)
    assert.equal(result, 10)
  })
  ...
  it('instance#hooks', async () => {
    ...
    let result = await resource.find('123')
