...
JsonStore.prototype.read = function (query, options) {
  // logger.warn(`json_store - READ ----------------`, query, options)
  return this._db.createReadStream({
    start: '\x00',
    end: '\xFF',
    keys: false,
...
JsonStore.prototype.find = function (id, callback) {
  // logger.warn(`json_store - FIND ---------`, id)
  this._db.get(id, (error, result) => {
    // logger.warn(`json_store - FIND result ---------`, id, result, error)
    if (error) {
      return callback({ error })
    }
...
JsonStore.prototype.update = function (id, obj, callback) {
  ...
  this._db.get(id, (error, result) => {
    ...
    // logger.warn(`json_store - UPDATE --------------------`, obj)
    self._db.put(id, JSON.stringify(obj), { sync: true }, (error) => {
      if (error) {
        return callback({ error })
      }
