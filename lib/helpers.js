...
class Helpers {
  ...
  merge(target, object) {
    _.each(object, (value, key) => {
      target[key] = (_.isObject(value) && !_.isArray(value))
        ? this.merge((target[key] || {}), value) : value
    })
    return target
  }
  ...
  async findRecord(context) {
    try {
      ...
      if (error.notFound || (error.error && error.error.notFound)) {
        return context.error(this.recordNotFoundErrorV2(context.resource.name, context.params.id, error))
      }
      return context.error(this.findRecordErrorV2(context.resource.name, context.params.id, error))
    }
  }
  ...
  findRecordAttachment(context) {
    ...
    if (!context.attachment) {
      context.error(this.findAttachmentErrorV2(context.resource.name, context.params.id, context.params.aid))
    } else {
      context.next()
    }
  }
  ...
  listRecordErrorV2(resource, query) {
  ...
  listNotFoundErrorV2(resource, query) {
  ...
  recordNotFoundErrorV2(resource, query) {
  ...
  findRecordErrorV2(resource, query) {
