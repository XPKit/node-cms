...
class Helpers {
  ...
  enableFirstAsActive = async (context) => {
    ...
    try {
      ...
      const otherRecords = await context.options.resource.list(
        { _id: { $not: { $eq: currentId } } },
        { page: 0, limit: 1 },
      )
  ...
  leaveOneActive = async (context) => {
    ...
    const activeField = _.get(context, 'options.activeField')
    if (
      _.isUndefined(currentId) ||
      _.isUndefined(activeField) ||
      _.get(context.params.object, `${activeField}`, false) !== true
    ) {
      return context.next()
    }
  ...
  normalizeSchema = (context) => {
    ...
    const localised = !!locales?.length
    if (!localised) {
      return context.next()
    }
    if (context.params.options?.locale) {
      // has locale ?locale=enUS { title: 'ABC', category: 123 }
      const newObject = {}
  ...
  getDependencyItems = async (context) => {
    try {
      ...
      await pAll(
        _.map(this.resolveMap, (item, key) => {
          return async () => {
  ...
  setValueInItem = (item, dependencyMap, field, fieldKey) => {
    ...
    if (_.isUndefined(value)) {
      ...
      return _.set(
        item,
        fieldKey,
        _.map(value, (id) => dependencyMap[field.source][id]),
      )
    }
  }
  injectDependencyByItem = (item, dependencyMap, schema, options) => {
    _.each(schema, (field) => {
      if (
        !_.includes(['select', 'multiselect'], field.input) ||
  ...
  listRecordError(resource, query) {
    return {
      code: 500,
      message: `list resource (${this.queryToLog(query)}) error in resource (${resource})`,
      data: arguments,
    }
  }
  listNotFoundError(resource, query) {
    return {
      code: 404,
      message: `list resource with query (${this.queryToLog(query)}) not found in resource (${resource})`,
      data: arguments,
    }
  }
  recordNotFoundError(resource, query) {
    return {
      code: 404,
      message: `record (${this.queryToLog(query)}) not found in resource (${resource}) error`,
      data: arguments,
    }
  }
  ...
  removeAttachmentError(resource, id, aid) {
    return {
      code: 500,
      message: `record (${id}) remove attachment (${aid}) in resource (${resource}) error`,
      data: arguments,
    }
  }
  findAttachmentError(resource, id, aid) {
    return {
      code: 404,
      message: `record (${id}) find attachment (${aid}) in resource (${resource}) error`,
      data: arguments,
    }
  }
