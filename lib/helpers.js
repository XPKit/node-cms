...
// Where uuid.js puts the machine id in a record id: after the eight characters of
 ...
 */
  resizeOptionsValid = (resizeOptions) => {
    ...
    return resizeOptionsRegex.test(resizeOptions)
  }
  ...
  foreignRecordError (resource, id) {
    return { code: 403, message: `Can't modify foreign record '${id}' in resource '${resource}'`, data: arguments }
  }
  /*
   * Whether a record id carries this server's machine id, and so belongs to it.
   *
   * uuid.js writes an id as `Date.now().toString(36)` - eight characters - then the eight-character
   * machine id, so the mark sits at MID_OFFSET and nowhere else counts. Every caller used to spell
   * this as `id.indexOf(mid) === 8`, which is a different question: indexOf answers with the first
   * occurrence, so a timestamp ending in '42' beat the mid 42424242 to it and the server disowned a
   * record it had just written - one id in 1296 (#105). It lives here so there is one of it.
   *
   * @param {String} id
   * @param {String} mid, this server's 8 character machine id
   * @return {Boolean} whether the record is this server's own
   */
  isOwnRecord (id, mid) {
    return _.isString(id) && _.isString(mid) && id.startsWith(mid, MID_OFFSET)
  }
  databaseError () {
