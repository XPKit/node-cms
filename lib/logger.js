...
_.set(process, 'env.DEBUG_HIDE_DATE', 'true')
...
if (_.get(process, 'env.DEBUG', false) === false) {
  ...
  console.log('log filter is: ', _.get(process, 'env.DEBUG', false))
}
...
class Logger {
  ...
  write (...args) {
    ...
    fs.ensureFile(logPath)
      .then(() => fs.appendFile(logPath, `${args.toString()}\n`))
      .catch(error => {
        process.stderr.write(`Didn't succeed to write logs: ${error.message}`)
        process.exit()
      })
  }

  timestamp () {
    return new Dayjs().format('YYYY/MM/DD HH:mm:ss.SSSS')
  }
