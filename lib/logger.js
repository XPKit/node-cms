...
if (_.isEmpty(_.get(process, 'env.DEBUG', ''))) {
  ...
  // enable() also persists the filter through debug's save(), which writes process.env.DEBUG. The
  // filter is already applied in memory, so put back exactly what the host had, which may be an
  // intentional empty string rather than nothing at all.
  if (_.isUndefined(previousDebug)) {
...
function callSites (boundary) {
  ...
  try {
    ...
    // Restored even if reading the stack throws; leaving this replaced would break stack
