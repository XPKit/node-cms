...
class ImageOptimization {
  ...
  /**
   * Load the ESM-only file-type package once (it ships no CommonJS build since v17)
   * @returns {Promise<Object>} The file-type module namespace
   */
  loadFileType() {
    if (!this.fileTypeModule) {
      this.fileTypeModule = import('file-type')
    }
    return this.fileTypeModule
  }

  /**
   * Detect the file type of a buffer from its magic number
   * @param {Buffer} buffer - File data, at least the first few kilobytes
   * @returns {Promise<{ext: string, mime: string}|undefined>} Detected type, undefined when unknown
   */
  async getFileType(buffer) {
    const { fileTypeFromBuffer } = await this.loadFileType()
    return fileTypeFromBuffer(buffer)
  }

  /**
   * Detect the file type of a file on disk from its magic number
   * @param {string} filePath - Path to the file
   * @returns {Promise<{ext: string, mime: string}|undefined>} Detected type, undefined when unknown
   */
  async getFileTypeFromFile(filePath) {
    const { fileTypeFromFile } = await this.loadFileType()
