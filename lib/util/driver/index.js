...
class Driver {
  ...
  static async _readImpl(context) {
    ...
    _.each(results, (record) => {
      record._local = h.isOwnRecord(_.get(record, '_id', ''), mid)
    })
    ...
    if (_.isString(params.query)) {
      try {
        ...
        if (result && _.isObject(result) && result._id) {
          result._local = h.isOwnRecord(_.get(result, '_id', ''), context.options.cms.mid)
        }
  ...
  static async _updateImpl(context) {
    ...
    try {
      ...
      // A structured error already carries the status it means - a refusal is a 403, not a
      // database failure - so wrapping it in a 500 would throw that away (#105).
      return context.error(_.get(error, 'code', false) ? error : h.updateRecordError(context.resource.name, context.params.id, error))
    }
  }
  ...
  static async _removeImpl(context) {
    try {
      ...
      return context.error(_.get(error, 'code', false) ? error : h.removeRecordError(context.resource.name, context.params.id, error))
    }
  }
  ...
  static async _findAttachmentImpl(context) {
    ...
    if (_.has(field, 'options.oss') && _.get(field, 'options.oss.accessKeyId', false)) {
