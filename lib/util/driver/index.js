...
class Driver {
  ...
  static async cleanAttachmentCache(resource, attachmentId) {
    return new Promise((resolve, reject) => {
      resource.file.list((error, aidList) => {
        if (error) {
          return reject(error)
        }
        // Find all cached versions of this attachment (files that start with attachmentId-)
        ...
        pAll(_.map(cachedVersions, cacheId => {
          return async () => {
            try {
              ...
              // Don't throw error for cache cleanup failures
            }
          }
        }), {concurrency: 5}).then(() => {
          resolve()
  ...
  // =============================================================================
  // Resource Operation Implementation Methods (formerly driver.define methods)
  // =============================================================================
  ...
  static async _readImpl(context) {
    return new Promise((resolve) => {
      context.stream = context.stream.pipe(through(function (data) {
        data._local = _.get(data, '_id', '').indexOf(context.options.cms.mid) === 8
  ...
  static async _listImpl(context) {
    ...
    try {
      const gotStream = await new Promise((resolve, reject) => {
        context.methods.read(context.params.query, context.params.options, (error, stream) => {
      ...
      gotStream.pipe(through(
        data => {
          return results.push(h.injectDependency(data, context.dependencyMap, context.resource.options, context.resource.resolveMap))
        },
        () => {
  ...
  static async _findImpl(context) {
    ...
    try {
      const results = await new Promise((resolve, reject) => {
        context.methods.list(context.params.query, _.extend(context.params.options || {}, {page: 0, limit: 1}), (error, result) => {
          if (error) reject(error)
          else resolve(result)
        })
      })
      if (!results.length) {
        return context.error(h.listNotFoundError(context.resource.name, context.params.query))
      }
      return context.result(results[0])
    } catch (error) {
      return context.error(h.listRecordError(context.resource.name, context.params.query, error))
    }
  }
    ...
    if (resizeOptions && _.includes(['image/jpeg', 'image/gif', 'image/png'], contentType)) {
      ...
      if (smartOptions) {
        ...
        if (fs.existsSync(smartCacheFile)) {
          ...
          try {
            const result = await ImageMin.smartCropAttachment(context.attachment.stream, resizeOptions, {
              detectFaces: true,
              minScale: 0.8,
            ...
            try {
              ...
              const result = await ImageMin.resizeAttachment(freshStream, resizeOptions)
      ...
      } else if (resizeOptions === defaultResize) {
        // Default resize handling
        const defaultCacheFile = path.join(context.resource.file._dir, `${context.attachment._id}-${defaultResize}`)

        if (fs.existsSync(defaultCacheFile)) {
          context.attachment.stream = context.resource.file.read(`${context.attachment._id}-${defaultResize}`)
        } else {
          console.info(`Couldn't find the ${context.attachment._id}-${defaultResize}, will send back the original attachment instead`)
        }
      } else {
        ...
        if (fs.existsSync(cacheFile)) {
          ...
          try {
            const result = await ImageMin.resizeAttachment(context.attachment.stream, resizeOptions)
