...
class Driver {
      ...
      api[name] = function () {
        return new Promise((resolve, reject) => {
          ...
          context.on('end', () => {
            ...
            // Execute after hooks
            const afterHooks = [].concat(method.after, hooks.after[name] || [], hooks.afterAll, self._hooks.after)
  ...
  static async cleanAttachmentCache(resource, attachmentId) {
    ...
    const cachedVersions = _.filter(aidList, (aid) => aid.startsWith(`${attachmentId}-`) && aid !== attachmentId)
    ...
    if (_.isString(params.query)) {
      try {
        ...
        return context.result(
          h.injectDependency(result, context.dependencyMap, context.resource.options, context.resource.resolveMap),
        )
      } catch (error) {
  ...
  static async _findAttachmentImpl(context) {
    ...
    if (
      resizeOptions &&
      h.resizeOptionsValid(resizeOptions) &&
      _.includes(['image/jpeg', 'image/gif', 'image/png'], contentType)
    ) {
      ...
      if (_.get(context, 'params.opts.smart', false)) {
        ...
        if (fs.existsSync(smartCacheFile)) {
          ...
          logger.info(
            `Smart crop cache miss for ${smartCacheKey} @ ${smartCacheFile}, creating and caching smart cropped image`,
          )
          try {
            const result = await ImageOptimization.smartCropAttachment(context.attachment.stream, resizeOptions, {
              detectFaces: true,
              minScale: 0.8,
              faceOnly: _.get(context, 'params.opts.faceOnly', false),
              facePadding: _.get(context, 'params.opts.facePadding', 10),
              objectDetection: _.get(context, 'params.opts.objectDetection', false),
            })
  ...
  static async _removeAttachmentImpl(context) {
    context.record._attachments = _.filter(
      context.record._attachments,
      (attachment) => attachment._id !== context.attachment._id,
    )
      ...
      stream
        .on('data', (data) => {
          ...
          if (_.isArray(attachments)) {
            ...
            aidList = _.reject(aidList, (item) => _.includes(list, item))
            return aidList
          }
          return []
        })
        ...
        .on('end', async () => {
          if (aidList.length > 0) {
            logger.info('removing %s attachments ... ...', aidList.length)
            let count = 0
            try {
              await pAll(
                _.map(aidList, (aid) => {
                  return async () => {
                    logger.info('removing %s ... ...', aid)
                    try {
                      await context.resource.file.remove(aid)
                      logger.info('removing %s ... ... done (%s/%s)', aid, ++count, aidList.length)
                    } catch (error) {
 ...
 */
module.exports = (name, options) => new Driver(name, options)
