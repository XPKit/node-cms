...
class SmartCrop {
   ...
   */
  async initialize(_cmsConfig = null) {
  ...
  //     let tf, blazeface, cocoSsd
  //     try {
  //       tf = require('@tensorflow/tfjs-node')
  //     } catch {
  //       logger.error('TensorFlow.js not found. To use smart cropping, please add in your package.json:')
  //       logger.error('"node-cms-tf": "file:node_modules/node-cms/tf"')
  //       logger.error('and run:')
  //       logger.error('npm i && npm i node-cms-tf')
  //       throw new Error('TensorFlow.js not installed. Smart cropping requires additional dependencies.')
  //     }
  //     await tf.ready()
  ...
  //     } catch {
  //       logger.error('Object detection model not found. To use object detection, please run:')
  //       logger.error('  npm install @tensorflow-models/coco-ssd')
  //       this.objectDetectionAvailable = false
  //       this.objectDetector = null
  //     }
  //     this.isInitialized = true
  ...
  isKnownBlazeFaceError(error) {
    return !!_.find(this.knownBlazeFaceErrors, (err) => error.message.includes(err))
  }
  ...
  async smartCrop(imageBuffer, targetSize, options = {}) {
    ...
    console.warn(
      `smartcrop debug ----- ${targetSize}`,
      options,
      this.faceDetectionAvailable,
      this.objectDetectionAvailable,
    )
  ...
  cropAroundObjects(objects, imageWidth, imageHeight, targetWidth, targetHeight) {
    ...
    const primaryObject = objects.reduce((prev, current) => (prev.score > current.score ? prev : current))
    ...
    const cropDimension = Math.max(objWidth, objHeight) + padding
    // Adjust for target aspect ratio
    let cropWidth, cropHeight
    if (Math.abs(targetRatio - 1) < 0.1) {
      // Square target
    ...
    logger.info(
      `Object-focused crop: ${Math.round(cropWidth)}x${Math.round(cropHeight)} at (${Math.round(cropX)}, ${Math.round(cropY)})`,
    )
    return {
      x: Math.round(cropX),
      y: Math.round(cropY),
  ...
  cropAroundMultiplePeople(people, imageWidth, imageHeight, targetWidth, targetHeight) {
    ...
    logger.info(
      `People bounding box: x=${Math.round(minX)}, y=${Math.round(minY)}, width=${Math.round(maxX - minX)}, height=${Math.round(maxY - minY)}`,
    )
    ...
    // Create crop area maintaining target aspect ratio
    const targetRatio = targetWidth / targetHeight
    const cropDimension = Math.max(groupWidth, groupHeight) + padding
    // Adjust for target aspect ratio
    let cropWidth, cropHeight
    if (Math.abs(targetRatio - 1) < 0.1) {
      // Square target
    ...
    cropHeight = Math.max(1, Math.min(cropHeight, imageHeight - cropY))
    logger.info(
      `Multiple people crop: ${Math.round(cropWidth)}x${Math.round(cropHeight)} at (${Math.round(cropX)}, ${Math.round(cropY)})`,
    )
    return {
      x: Math.round(cropX),
      y: Math.round(cropY),
  ...
  async applyCrop(imageBuffer, targetSize, options = {}) {
    try {
      ...
      try {
        ...
        const buffer = await image.toBuffer()
        logger.info(
          `Applied fallback center crop: ${Math.round(crop.width)}x${Math.round(crop.height)} at (${Math.round(crop.x)}, ${Math.round(crop.y)})`,
        )
        return {
          buffer,
          mimeType: metadata.format ? `image/${metadata.format}` : 'image/jpeg',
  ...
  cropFaceOnly(faces, imageWidth, imageHeight, targetWidth, targetHeight, facePadding = 10) {
    ...
    logger.info(
      `Detected face at: x=${Math.round(faceX)}, y=${Math.round(faceY)}, width=${Math.round(faceWidth)}, height=${Math.round(faceHeight)}`,
    )
    ...
    cropHeight = cropSize
    logger.info(
      `Square crop with ${padding}px face padding: ${Math.round(cropSize)}x${Math.round(cropSize)} at (${Math.round(cropX)}, ${Math.round(cropY)})`,
    )
    // Ensure crop stays within image bounds
    if (cropX < 0) {
      cropX = 0
    ...
    cropHeight = Math.max(1, Math.min(cropHeight, imageHeight - cropY))
    logger.info(
      `Final face-only crop (square): ${Math.round(cropWidth)}x${Math.round(cropHeight)} at (${Math.round(cropX)}, ${Math.round(cropY)})`,
    )
    return {
      x: Math.round(cropX),
      y: Math.round(cropY),
