...
// TensorFlow.js modules will be dynamically imported when needed
let tf = null
let blazeface = null
let cocoSsd = null
...
class SmartCrop {
  ...
  async initialize(cmsConfig = null) {
    ...
    if (!smartCropEnabled) {
      console.info('Smart cropping disabled in configuration - face and object detection will not be available')
      this.faceDetector = null
      this.objectDetector = null
      this.faceDetectionAvailable = false
      this.objectDetectionAvailable = false
    ...
    try {
      ...
      console.info('Smart cropping enabled - loading TensorFlow.js Node backend...')

      // Dynamically import TensorFlow.js modules
      tf = (await import('@tensorflow/tfjs-node')).default
      blazeface = (await import('@tensorflow-models/blazeface')).default
      cocoSsd = (await import('@tensorflow-models/coco-ssd')).default

      console.info('Initializing TensorFlow.js Node.js backend...')
      ...
      try {
        ...
        console.warn('Face detection initialization failed, will fallback to object detection only')
        this.faceDetectionAvailable = false
        this.faceDetector = null
      }
      // Initialize object detection
      console.info('Initializing object detection model...')
      // cocoSsd is dynamically imported above
      this.objectDetector = await cocoSsd.load()
      this.objectDetectionAvailable = true
      console.info('Object detection model initialized successfully')
      this.isInitialized = true
    } catch (error) {
      console.warn('Detection models initialization failed:', error.message)
      console.info('Smart cropping will use intelligent center crop instead')
      this.faceDetector = null
      this.objectDetector = null
      this.faceDetectionAvailable = false
      this.objectDetectionAvailable = false
  ...
  async detectObjects(imageBuffer) {
    ...
    try {
      ...
      console.info(`Detected ${objects.length} object(s)`)
      if (objects.length > 0) {
        console.info('Object detection results:', objects.map(obj => ({
          class: obj.class,
      ...
      if (error.message.includes('Kernel') || error.message.includes('not registered')) {
        console.warn('Object detection runtime error detected, disabling object detection for this session')
        this.objectDetectionAvailable = false
        this.objectDetector = null
      } else {
  ...
  async detectFaces(imageBuffer) {
    ...
    try {
      ...
      console.info(`Detected ${faces.length} face(s)`)
      if (faces.length > 0) {
        ...
        const standardizedFaces = faces.map(face => {
          if (face.topLeft && face.bottomRight) {
            ...
            console.warn('Unknown face detection format:', face)
            return null
          }
        }).filter(face => face !== null)

        console.info('Face detection result:', JSON.stringify(standardizedFaces[0].box, null, 2))
      ...
      if (error.message.includes('Kernel') ||
        ...
        console.warn('Face detection runtime error detected, disabling face detection for this session')
        this.faceDetectionAvailable = false
        this.faceDetector = null
      } else {
    ...
    try {
      ...
      if (opts.faceOnly) {
        if (this.faceDetectionAvailable) {
          ...
          if (faces.length > 0) {
            console.info(`Creating face-only crop from ${faces.length} detected face(s)`)
      ...
      if (opts.objectDetection) {
        if (this.objectDetectionAvailable) {
          ...
          if (objects.length > 0) {
            console.info(`Creating object-focused crop from ${objects.length} detected object(s)`)
      ...
      if (this.objectDetectionAvailable) {
        ...
        if (objects.length > 0) {
          ...
          if (people.length > 1) {
            ...
            console.info(`Creating crop around ${people.length} detected people`)
            ...
            if (opts.faceOnly) {
              ...
              if (this.faceDetectionAvailable) {
                const faces = await this.detectFaces(imageBuffer)
                if (faces.length > 0) {
                  console.info(`Creating face-only crop from ${faces.length} detected face(s) (single person detected)`)
                  return this.cropFaceOnly(faces, image.width, image.height, targetWidth, targetHeight, opts.facePadding)
                }
              }
              // Fall back to cropping around the person if face detection fails
              console.info('Face detection failed for single person, cropping around person object')
              ...
              console.info('Creating crop around single detected person')
              return this.cropAroundObjects(people, image.width, image.height, targetWidth, targetHeight)
            }
          } else {
            // No people detected - crop around any detected objects
            console.info(`No people detected, creating crop around ${objects.length} detected object(s)`)
            return this.cropAroundObjects(objects, image.width, image.height, targetWidth, targetHeight)
          }
        }
      }
      ...
      if (opts.detectFaces && this.faceDetectionAvailable) {
        const faces = await this.detectFaces(imageBuffer)
        if (faces.length > 0) {
          console.info(`Using ${faces.length} face(s) to guide smart cropping`)
      ...
      console.info('Using intelligent center crop algorithm')
  ...
  cropAroundObjects(objects, imageWidth, imageHeight, targetWidth, targetHeight) {
    if (objects.length === 0) {
      console.warn('No objects detected for object-focused crop, falling back to center crop')
    ...
    console.info(`Primary object: ${primaryObject.class} (confidence: ${primaryObject.score.toFixed(3)})`)
    console.info(`Object bbox: x=${Math.round(objX)}, y=${Math.round(objY)}, width=${Math.round(objWidth)}, height=${Math.round(objHeight)}`)
    ...
    console.info(`Object-focused crop: ${Math.round(cropWidth)}x${Math.round(cropHeight)} at (${Math.round(cropX)}, ${Math.round(cropY)})`)
    return {
      x: Math.round(cropX),
      y: Math.round(cropY),
  ...
  cropAroundMultiplePeople(people, imageWidth, imageHeight, targetWidth, targetHeight) {
    if (people.length === 0) {
      console.warn('No people detected for multiple people crop, falling back to center crop')
      return this.calculateCenterCrop(imageWidth, imageHeight, targetWidth, targetHeight)
    }
    ...
    console.info(`People bounding box: x=${Math.round(minX)}, y=${Math.round(minY)}, width=${Math.round(maxX - minX)}, height=${Math.round(maxY - minY)}`)
    ...
    cropWidth = Math.max(1, Math.min(cropWidth, imageWidth - cropX))
    cropHeight = Math.max(1, Math.min(cropHeight, imageHeight - cropY))
    console.info(`Multiple people crop: ${Math.round(cropWidth)}x${Math.round(cropHeight)} at (${Math.round(cropX)}, ${Math.round(cropY)})`)
    return {
      x: Math.round(cropX),
      y: Math.round(cropY),
      width: Math.round(cropWidth),
  ...
  /**
   * Apply smart crop to an image buffer using Jimp
   * @param {Buffer} imageBuffer - The image buffer to crop
   * @param {string} targetSize - Target size in format "WIDTHxHEIGHT"
   * @param {Object} [options] - Additional options
  ...
  async applyCrop(imageBuffer, targetSize, options = {}) {
    ...
    try {
      ...
      // Apply crop
      image.crop(cropResult.x, cropResult.y, cropResult.width, cropResult.height)
      ...
      return {
        ...
        cropResult,
        originalSize: {
          width: (await Jimp.read(imageBuffer)).getWidth(),
          height: (await Jimp.read(imageBuffer)).getHeight()
        },
        targetSize: { width: targetWidth, height: targetHeight }
      }
    } catch (error) {
      console.error('Smart crop failed, falling back to center crop:', error)
      ...
      try {
        ...
        const sourceRatio = image.getWidth() / image.getHeight()
        const targetRatio = targetWidth / targetHeight
        ...
        if (sourceRatio > targetRatio) {
          ...
          cropWidth = image.getHeight() * targetRatio
          cropX = (image.getWidth() - cropWidth) / 2
        } else {
          // Source is taller than target - crop height
          cropHeight = image.getWidth() / targetRatio
          cropY = (image.getHeight() - cropHeight) / 2
        }
        // Apply center crop
        image.crop(Math.round(cropX), Math.round(cropY), Math.round(cropWidth), Math.round(cropHeight))
        // Resize to target dimensions
        image.resize(targetWidth, targetHeight, Jimp.RESIZE_BILINEAR)
        const buffer = await image.getBufferAsync(image.getMIME())
        console.info(`Applied fallback center crop: ${Math.round(cropWidth)}x${Math.round(cropHeight)} at (${Math.round(cropX)}, ${Math.round(cropY)})`)
        return {
          buffer,
          mimeType: image.getMIME(),
          contentLength: buffer.length,
          ...
          originalSize: { width: image.getWidth(), height: image.getHeight() },
          targetSize: { width: targetWidth, height: targetHeight },
          fallback: true
        }
      } catch (fallbackError) {
        console.error('Fallback center crop also failed:', fallbackError)
  ...
  async detectBothFacesAndObjects(imageBuffer) {
    ...
    try {
      ...
      console.info(`Detected ${faces.length} face(s) and ${objects.length} object(s) in one pass`)
      return { faces, objects }
    } catch (error) {
      console.warn('Combined detection failed:', error.message)
      return { faces: [], objects: [] }
    }
  }
  ...
  cropFaceOnly(faces, imageWidth, imageHeight, targetWidth, targetHeight, facePadding = 10) {
    if (faces.length === 0) {
      console.warn('No faces detected for face-only crop, falling back to center crop')
      return this.calculateCenterCrop(imageWidth, imageHeight, targetWidth, targetHeight)
    }
    ...
    console.info(`Detected face at: x=${Math.round(faceX)}, y=${Math.round(faceY)}, width=${Math.round(faceWidth)}, height=${Math.round(faceHeight)}`)
    console.info(`Image dimensions: ${imageWidth}x${imageHeight}`)
    ...
    console.info(`Square crop with ${padding}px face padding: ${Math.round(cropSize)}x${Math.round(cropSize)} at (${Math.round(cropX)}, ${Math.round(cropY)})`)
    // Ensure crop stays within image bounds
    if (cropX < 0) {
      cropX = 0
    } else if (cropX + cropWidth > imageWidth) {
    ...
    cropWidth = Math.max(1, Math.min(cropWidth, imageWidth - cropX))
    cropHeight = Math.max(1, Math.min(cropHeight, imageHeight - cropY))
    console.info(`Final face-only crop (square): ${Math.round(cropWidth)}x${Math.round(cropHeight)} at (${Math.round(cropX)}, ${Math.round(cropY)})`)
    return {
      x: Math.round(cropX),
      y: Math.round(cropY),
      width: Math.round(cropWidth),
