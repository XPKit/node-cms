...
class SmartCrop {
  ...
  async smartCrop(imageBuffer, targetSize, options = {}) {
    ...
    try {
      ...
      // if targetWidth or targetHeight is "auto", calculate the value based on the ratio of the image and the other value provided
      const widthNumber = _.toNumber(targetWidth)
      if (!_.isNaN(widthNumber)) {
        targetWidth = widthNumber
      }
      const heightNumber = _.toNumber(targetHeight)
      if (!_.isNaN(heightNumber)) {
