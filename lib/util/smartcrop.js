...
class SmartCrop {
  ...
  async smartCrop(imageBuffer, targetSize, options = {}) {
    ...
    // Order of priority of smart cropping
    ...
    // 3. default cropping (no specific options)
    console.warn('SMART CROP OPTIONS=', opts)
    try {
      const image = await loadImage(imageBuffer)
      const {targetWidth, targetHeight} = this.getSizes(image, targetSize)
      console.warn(`sizes are now ${targetWidth}x${targetHeight}`)
        ...
        if (objects.length > 0) {
          ...
          if (people.length > 1) {
            ...
            if (opts.faceOnly) {
              ...
              logger.info('Face detection failed for single person, will crop around person object')
            } else {
