...
class XlsxManager {
  ...
  initialize = () => {
    ...
    this.app.get('/:resource', mw.find_resource(this), this.authorize, this.onGet)
    this.app.post('/:resource/status', mw.find_resource(this), this.authorize, upload.single('xlsx'), this.injectImportMap, this.onPostStatus)
    this.app.post('/:resource/import', mw.find_resource(this), this.authorize, upload.single('xlsx'), this.injectImportMap, this.onPostImport)
  ...
  onPostImport = async (req, res, next) => {
    try {
  ...
  onGet = async (req, res, next) => {
    try {
      ...
      const rows = [_.map(keyList, 'label'), _.map(keyList, 'key')]
      _.each(items, item => {
        rows.push(_.map(keyList, keyItem => this.getExportCellValue(item, keyItem)))
      })
      xlsx.utils.book_append_sheet(wb, this.rowsToSheet(rows), req.params.resource)
      // add extra page
      ...
      await pAll(_.map(relatedResources, resource => {
        return async () => {
          ...
          const locales = options.locales
          const relativeKeyList = _.clone(this.api(resource).getUniqueKeys())
          ...
          const relatedItems = await this.api(resource).list()
          const relatedRows = [relativeKeyList]
          _.each(relatedItems, item => {
            relatedRows.push(_.map(relativeKeyList, key => this.toExportPrimitive(_.get(item, key, undefined))))
          })
          xlsx.utils.book_append_sheet(wb, this.rowsToSheet(relatedRows), resource)
        }
      }), {concurrency: 1})
  ...
  /**
   * Builds a worksheet from rows of primitive values. Date cells get a yyyy-mm-dd number format.
   * SheetJS Community Edition writes values and number formats only, so the header fills and the
   * "locked cell" validations the former excel4node export produced are not reproduced.
   * @param {Array<Array>} rows - Rows of cell values; undefined leaves a cell empty
   * @returns {Object} SheetJS worksheet
   */
  rowsToSheet (rows) {
    return xlsx.utils.aoa_to_sheet(rows, {dateNF: 'yyyy-mm-dd'})
  }

  /**
   * Coerces a record value to what a cell can hold: booleans, numbers and strings pass through,
   * anything else (null, objects) becomes an empty string, as the previous export did.
   * @param {*} value - Record field value
   * @returns {boolean|number|string|undefined} Cell value; undefined leaves the cell empty
   */
  toExportPrimitive (value) {
    if (_.isUndefined(value) || _.isBoolean(value) || _.isNumber(value) || _.isString(value)) {
      return value
    }
    return ''
  }
  ...
  /**
   * Resolves the cell value of one export column for one record: objects as JSON, arrays joined
   * with commas, attachments as their URL and date-like numbers as Date objects.
   *
   * A wysiwyg field always exports an empty cell. Its markup cannot be represented in one cell, and
   * anything written here comes straight back through injectImportMap on the next import and replaces
   * the stored markup. An absent cell never reaches the import, so the field is left untouched, which
   * matches how file and image columns are already skipped.
   * @param {Object} item - Record prepared by onGet, relations already resolved
   * @param {Object} keyItem - Column descriptor with key, field and optional locale
   * @returns {boolean|number|Date|string|undefined} Cell value; undefined leaves the cell empty
   */
  getExportCellValue (item, keyItem) {
    const inputType = keyItem.field.input
    let value = _.get(item, keyItem.key, undefined)
    if (inputType === 'object' && value) {
      value = JSON.stringify(value)
    } else if (_.includes(['multiselect', 'pillbox'], inputType) && _.isArray(value)) {
      value = value.join(',')
    } else if (_.includes(['file', 'image'], inputType)) {
      const attach = _.find(item._attachments, attach => {
        return (attach._name === keyItem.field.field) && (!keyItem.locale || keyItem.locale === attach._fields.locale)
      })
      value = _.get(attach, 'url', undefined)
    }
    if (inputType === 'wysiwyg') {
      return undefined
    }
    if (_.isNumber(value) && _.includes(['date', 'datetime', 'time'], inputType)) {
      return new Date(value)
    }
    return this.toExportPrimitive(value)
  }
