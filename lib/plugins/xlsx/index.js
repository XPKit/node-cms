...
class XlsxManager {
  ...
  async initialize () {
    const { default: find_resource } = await import('../rest/middleware/find_resource.js')
    const mw = {find_resource}
    ...
    try {
      ...
      _.each(keyList, (keyItem, idx) => {
        ...
        _.each(items, (item, itemIdx) => {
          let value
          let attach
          if (keyItem.field.input === 'object') {
            value = _.get(item, keyItem.key)
            if (value) {
              value = JSON.stringify(value)
            }
          } else if (keyItem.field.input === 'multiselect' || keyItem.field.input === 'pillbox') {
            value = _.get(item, keyItem.key)
            if (_.isArray(value)) {
              value = value.join(',')
            }
          } else if (keyItem.field.input === 'file' || keyItem.field.input === 'image') {
