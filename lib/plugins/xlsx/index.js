...
class XlsxManager {
    ...
    return (item) => {
      ...
      _.each(options.schema, (field) => {
        ...
        _.each(localeArray, (locale) => {
          ...
          const regex = _.get(field, 'options.regex.value')
          if (regex) {
            if (!regex.test(value)) {
              invalidFormatErrorList.push(label)
              isValid = false
      ...
      if (!_.isEmpty(invalidFormatErrorList)) {
        errors.push({
          type: 'invalid',
          message: `${item._rowInfo}: Please enter a valid ${invalidFormatErrorList.join(', ')}.`,
        })
      }
      return isValid
    }
  ...
  injectImportMap = async (req, _res, next) => {
    try {
      if (!req.file) {
      ...
      const dataArray = []
      _.each(
        _.reject(_.keys(sheet), (item) => _.startsWith(item, '!')),
        (key) => {
      ...
      importList = _.compact(
        _.map(importList, (item) => {
          const obj = {}
      ...
      _.each(importListMap, (list, key) => {
        if (list.length > 1) {
          ...
          errors.push({
            type: 'duplicate',
            message: `${this.stringifyKeyObj(keyObj)} - ${_.map(list, '_rowInfo').join(', ')}`,
          })
  ...
  async onGet(req, res, next) {
    try {
            ...
            if (field.input === 'select') {
              ...
              _.each(items, (item, idx) => {
                ...
                _.each(localeArray, (locale) => {
                  ...
                  value = relatedItem?.[relationUniqueKey]
              ...
              _.each(items, (item, idx) => {
                ...
                _.each(localeArray, (locale) => {
                  ...
                  value = _.map(value, (id) => {
                    ...
                    return relatedItem?.[relationUniqueKey]
                  })
                  _.set(newItems, `${idx}.${fieldName}`, value)
                })
              })
              ...
              _.each(items, (item, idx) => {
                ...
                _.each(_.filter(item._attachments, { _name: field.field }), (attach) => {
                  ...
                  const url = `${serverUrl}/api/${req.params.resource}/${item._id}/attachments/${attach._id}`
        ...
        _.each(items, (item, itemIdx) => {
          ...
          if (inputType === 'object') {
            ...
            attach = _.find(item._attachments, (attach) => {
              return (
                attach._name === keyItem.field.field && (!keyItem.locale || keyItem.locale === attach._fields.locale)
              )
            })
            if (attach) {
              value = attach.url
      ...
      // add extra page
      const relatedResources = _.uniq(
        _.compact(_.map(options.schema, (field) => _.isString(field.source) && field.source)),
      )
      await pAll(
        _.map(relatedResources, (resource) => {
          return async () => {
