...
class MigrationManager {
  ...
  async preCheck (req, res, next) {
    try {
      ...
      let list = await promisifyMethod(fs, 'readdir')(this.folderPath)
      ...
      await pAll(_.map(list, item => {
        return async () => {
          ...
          try {
            let obj = await promisifyMethod(fs, 'readJson')(jsonPath)
      ...
      await pAll(_.map(req.migrationMap, (map, key) => {
        return async () => {
          try {
            ...
            await promisifyMethod(resource.json._db, 'batch')(batchList)
          } catch (error) {
            logger.warn(error.message)
          }
        }
