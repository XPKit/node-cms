...
const Admin = function (options) {
  ...
  return function () {
    ...
    admin.get('/login', async (req, res, next) => {
      ...
      // console.warn(`on get login ------`, _.get(req, 'session.nodeCmsUser', false))
      res.send({
        username: _.get(req, 'session.nodeCmsUser.username', false),
        group: _.get(req, 'session.nodeCmsUser.group.name', false)
      })
    })

    admin.get('/logout', async (req, res, next) => {
      logger.warn(`GET admin /logout`)
    ...
    admin.get('/resources',
      async (req, res, next) => {
        ...
        // console.warn(`-------- GET RESOURCES`, userGroup)
        if (userGroup) {
          const foundUserGroup = await cms.groups.find({_id: userGroup._id})
          // authed
          try {
