...
const Admin = function (options) {
  ...
  return function () {
    ...
    admin.get('/login', async (req, res, next) => {
      ...
      if (!group) {
        group = _.get(await cms.groups.find({_id: _.get(nodeCmsUser, 'group', false)}), 'name', false)
        _.set(req, 'session.nodeCmsUser.group', group)
      }
      res.status(200).send({
        username: _.get(nodeCmsUser, 'username', false),
        group
      })
    })
