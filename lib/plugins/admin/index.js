...
const Admin = function (options) {
  ...
  return function () {
    ...
    const config = _.pick(options, ['autoload', 'disableJwtLogin', 'disableAuthentication', 'apiVersion', 'blockRetry', 'import', 'syslog', 'sync', 'toolbarTitle', 'webserver'])
        ...
        let resources = _.map(cms._resources, (resource, name) => {
          _.each(resource.options, (item) => {
            if (_.isRegExp(_.get(item, 'options.regex.value'))) {
              ...
              _.forEach(item.options.regex, (localeItem) => {
