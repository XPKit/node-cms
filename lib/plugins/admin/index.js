...
class Admin {
        ...
        express.static(path.join(__dirname, '../../../public'), {
          ...
          setHeaders: (res, path) => {
            if (path.search('index.html') > -1) {
              ...
              res.setHeader('Last-Modified', new Date(Date.now() - Math.ceil(os.uptime())).toUTCString())
            }
          },
        }),
      )
  ...
  serveTemplate = (_req, res, url, userLoggedIn) => {
    const forDev = _.get(process, 'env.VITE_DEV_MODE', false)
    if (!forDev) {
  ...
  onPostLogin = async (req, res) => {
    const { error, result } = await this.cms.$authentication.authenticateJwt(
      req.body.username,
      req.body.password,
      req,
      res,
  ...
  onGetRoot = async (req, res, next) => {
    ...
    if (this.cms.$authentication.getUserFromToken && _.isEmpty(await this.cms.$authentication.getUserFromToken(req))) {
      ...
      return this.serveTemplate(
        req,
        res,
        `${`${req.protocol}://${req.get('host')}${req.originalUrl}`}index.html`,
        false,
      )
    }
  ...
  onGetChangeTheme = async (req, res) => {
    try {
      ...
      user = await this.cms.$authentication.users.find({
        username: _.get(user, 'username', false),
        group: _.get(user, 'group._id'),
      })
  ...
  onGetLogin = async (req, res, next) => {
    ...
    if (!_.get(this.options, 'disableJwtLogin', false)) {
      if (
        ...
        this.cms.$authentication.getUserFromToken &&
        _.isEmpty(await this.cms.$authentication.getUserFromToken(req))
      ) {
      ...
      try {
        ...
        const user = await this.cms.$authentication.users.find({
