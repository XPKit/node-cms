...
const Admin = function (options) {
  ...
  return function () {
    ...
    const handlePlugins = () => {
      let pluginsFolder = path.resolve('.', 'src', 'plugins')
      if (fs.existsSync(pluginsFolder)) {
        console.warn(`found plugins folder ${pluginsFolder}`)
        return admin.use('/plugins', express.static(pluginsFolder))
      }
      logger.info(`No plugins folder found @ ${pluginsFolder}`)
      const currentDir = __dirname
      const isDependency = currentDir.includes('/node_modules/node-cms/') || currentDir.includes('\\node_modules\\node-cms\\')
      console.warn('currentDir ? ', currentDir, isDependency)
      // fs.symlinkSync('')
      let symLinkTarget = path.resolve(currentDir, '../../..', 'src', '.plugins')
      if (isDependency) {
        const testPath = path.resolve('..', '..', 'plugins')
        if (fs.existsSync(testPath)) {
          symLinkTarget = testPath
        }
      }
      console.warn('Symlink target = ', symLinkTarget, ' : ', path.resolve('..', '..', 'plugins'), ' || ', path.resolve(currentDir, '../../..', 'src', '.plugins'))
      const symlinkPath = path.resolve(currentDir, '../../..', 'src', 'plugins')
      fs.symlinkSync(symLinkTarget, symlinkPath)
      console.warn(`Symlink created @ ${symlinkPath} -> ${symLinkTarget}`)
    ...
    this.serveDevMode = async function (req, res, next) {
      ...
      const url = `http://localhost:${devPort}${!userLoggedIn ? '/admin/index.html' : req.originalUrl}`
