...
const Admin = function (options, configPath) {
  ...
  return function () {
    ...
    const config = _.pick(options, ['autoload', 'disableJwtLogin', 'disableAuthentication', 'apiVersion', 'blockRetry', 'import', 'importFromRemote', 'syslog', 'sync', 'toolbarTitle', 'webserver', 'wsRecordUpdates'])
    ...
    if (!opts.disableJwtLogin) {
      admin.get('/', async (req, res, next) => {
        ...
        if (cms.getUserFromToken && _.isEmpty(await cms.getUserFromToken(req))) {
          ...
          return serveTemplate(req, res, `${req.protocol + '://' + req.get('host') + req.originalUrl}index.html`, false)
        }
        const url = req.protocol + '://' + req.get('host') + req.originalUrl
        if (_.endsWith(url, '/admin/')) {
          return serveTemplate(req, res, `${url}index.html`, true)
        }
        next()
      })
    ...
    this.checkIfUserLoggedIn = async function (req) {
      if (cms.getUserFromToken && _.isEmpty(await cms.getUserFromToken(req))) {
    ...
    const serveTemplate = function (req, res, url, userLoggedIn) {
      const forDev = _.get(process, 'env.VITE_DEV_MODE', false)
      if (!forDev) {
        ...
        return res.send(replaceTagsInTemplate(body, userLoggedIn))
      }
    ...
    admin.get('/changeTheme/:newTheme', async (req, res) => {
      try {
    ...
    admin.get('/_groups', async (req, res) => {
      let groups = await cms.groups.list({})
    ...
    if (fs.existsSync(i18nFolder)) {
      admin.get('/i18n/config.json', (req, res) => res.json(_.get(cms, '_options.admin', { language: { 'defaultLocale': 'enUS', 'locales': ['enUS'] } })))
    ...
    admin.get('/resources',
      ...
      async (req, res) => {
        ...
        let resources = _.map(cms._resources, (resource, name) => {
    ...
    admin.get('/paragraphs',
      ...
      async (req, res) => {
        try {
          ...
          req.paragraphs = cms._paragraphs
          const paragraphs = _.map(cms._paragraphs, (paragraph, name)=> {
    ...
    const isAdminUser = async (req) => {
      try {
        if (!options.disableJwtLogin) {
          const user = await cms.getUserFromToken(req)
    ...
    admin.get('/cms-config', async (req, res) => {
      try {
    ...
    admin.post('/cms-config', async (req, res) => {
      try {
        if (!(await isAdminUser(req))) {
          return res.status(403).json({ error: 'Access denied. Admin privileges required.' })
        }
