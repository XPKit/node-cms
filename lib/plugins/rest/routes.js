...
const injectAttachmentUrl = function (obj, req) {
  if (obj) {
    if (_.isArray(obj)) {
      ...
      const attachmentsFields = _.get(req, `resource.cms._attachmentFields.${req.resource.name}`, {})
      _.each(obj._attachments, (attachment) => {
        ...
        if (!hasField) {
          _.forEach(attachmentsFields, (attachmentsField, attachmentsFieldsRegEx) => {
            ...
            if (_.get(found, 'length', 0) > 0) {
              ...
              // console.warn('FOUND', rootPath, rootPathString, attachmentsField)
              return false
            }
          })
        }
        ...
        if (hasDirtyAttachments) {
          _.forEach(attachments, attachment => attachment.dirty = true)
        }
