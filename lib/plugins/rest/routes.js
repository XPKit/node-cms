  ...
  _.each(obj._attachments, (attachment) => {
    ...
    if (!hasField) {
      _.each(attachmentsFields, (attachmentsField, attachmentsFieldsRegEx) => {
        try {
          ...
          logger.error(
            `Failed to escapeRegExp for rootPath ${rootPath} in resource ${req.params.resource}`,
            error,
            attachment,
          )
        }
    ...
    if (hasDirtyAttachments) {
      _.each(attachments, (attachment) => {
        attachment.dirty = hasDirtyAttachments
      })
    }
    if (hasField) {
    ...
    if (_.get(attachment, '_name.length', 0) > 0) {
      _.set(
        ...
        attachment._name,
        _.uniqBy(
          _.concat(_.get(attachmentFieldsToMerge, attachment._name, []), attachments),
