...
exports.findCroppedAttachment = function (req, res) {
  ...
  req.resource.findAttachment(req.params.id, req.params.aid, async (error, result) => {
    ...
    if (cropOptions) {
      // console.warn('Crop options detected, will crop the image', cropOptions)
      const optimizedAttachment = await optimizeAttachment(result.stream, cropOptions)
      res.set({ 'Content-Length': optimizedAttachment.contentLength })
      res.type(optimizedAttachment.mimeType)
      // console.warn(`mimeType: ${optimizedAttachment.mimeType} - Content-Length: ${optimizedAttachment.contentLength}`)
      res.write('', 'binary') // it is trick by kong, handling some naughty unity3d file with browser
      optimizedAttachment.stream.pipe(res, { end: true })
      return
    }
    result.stream.on('end', () => {
      res.end()
