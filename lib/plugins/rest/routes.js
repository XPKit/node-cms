...
exports.create = async function (req, res) {
  ...
  try {
    console.warn('REST API: Creating resource record', { resource: req.resource && req.resource.name, body: req.body, options: req.options })
    const record = await req.resource.create(req.body, req.options)
    console.warn('REST API: Resource record created', { record })
    injectAttachmentUrl(record, req)
    res.json(record)
  } catch (error) {
  ...
  try {
    ...
    if (resizeOptions && h.resizeOptionsValid(resizeOptions) && _.includes(['image/jpeg', 'image/gif', 'image/png'], contentType)) {
      ...
      try {
        ...
        try {
          const resized = await ImageMin.resizeAttachment(result.stream, resizeOptions)
          ...
          // Send resized image
          const buffer = await ImageMin.getBufferFromStream(req.resource.file.read(cacheKey))
          res.type(contentType)
          return res.send(buffer)
        } catch (error) {
