...
const authorize = function (cms) {
  ...
  function (req, res, next) {
    cms.authorize(_.get(req, 'session.nodeCmsUser', false), req.resource, methods[req.method], req.originalUrl.indexOf('/attachments') !== -1, async (error) => {
      if (error) {
        logger.error('authorize Error: ', error)
      ...
      try {
        userGroup = _.get(await cms.groups.find({_id: userGroup}), 'name', userGroup)
      } catch {}
