...
const authorize = function (cms) {
  ...
  function (req, res, next) {
    cms.authorize(_.get(req, 'session.nodeCmsUser', false), req.resource, methods[req.method], req.originalUrl.indexOf('/attachments') !== -1, (error) => {
      if (error) {
        logger.error('authorize Error: ', error)
        return unauthorized(res)
      }
      _.set(req, 'body._updatedBy', _.get(req, 'session.nodeCmsUser.username', false))
      next()
    })
  }]
}
