...
class ReplicatorMongoDb {
  constructor (cms) {
    ...
    if (cms.options.netPort) {
      protocol.Server(cms.options.netPort, cms.options.mid, async (error, socket, name, remoteId) => {
        ...
        // send data to slave
        await Q.ninvoke(this.cms.resource(name).json, 'sync', socket, false, remoteId)
      })
    }
  }
  ...
  async replicate (host, port, baseUrl, name, cb, recordId) {
    try {
      ...
      if (recordId) {
        await Q.ninvoke(resource.json, 'sync', socket, true, remoteId, recordId)
      } else {
      ...
      return new EntryStream(resource.json._db, {
        ...
        const no = await Q.ninvoke(resource.json, 'cleanIndex')
        if (no > 0) {
