  ...
  async onPostLogin (req, res) {
    ...
    try {
      ...
      if (error) {
        ...
        logger.info('Login error response:', error)
      ...
      logger.info('Login success response:', req.session.nodeCmsUser)
    } catch (error) {
      logger.error('Error on login:', error.message)
      logger.info('Login catch error response:', error)
    ...
    logger.info('Final login response:', userToReturn)
