  ...
  return function () {
    ...
    const verifyToken = this.verifyToken = async (req, res, next = false) => {
      ...
      try {
        ...
        logger.error(`verifyToken - ${req.originalUrl} Error: `, error.message)
        return res.status(403).json({})
      }
      const token = req.body.token || req.query.token || req.headers['x-access-token'] || _.get(req, 'session.nodeCmsJwt') || _.get(req, 'cookies.nodeCmsJwt', false)
      if (!token) {
