...
class Authentication {
  ...
  verifyToken = async (req, res, next = false) => {
    // getTokenFromReq also accepts the token in the body, the query and the x-access-token header,
    // so rehydrating only from the cookie left those callers with an empty session. A username is
    // what makes a session user worth keeping: an anonymous request leaves one behind carrying only
    // the anonymous group, which authorize() writes into it, and that must not block a real token.
    if (!_.get(req, 'session.nodeCmsUser.username', false)) {
  ...
  authenticate = async (username, password, req, res, isJwt = false) => {
    ...
    let user = false
    try {
      user = await this.users.find({ username })
      if (!user) {
        throw new Error('User not found')
      }
    } catch {
