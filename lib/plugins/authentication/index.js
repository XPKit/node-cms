...
const generatePassword = function (str, salt) {
...
const Authentication = options => {
  ...
  return function () {
    ...
    let anonymousGroup
    let adminsGroup
    this.settings = this.resource('_settings', {
      displayname: {
        enUS: 'Settings',
        zhCN: '设置 '
      },
    ...
    const cleanRecord = (resource, toFind, next) => {
      resource.list(toFind, (error, result) => {
        if (error) {
          return next()
        }
        const q = queue(1)
        ...
        _.each(result, item =>
          q.defer(next =>
            resource.remove(item._id, (error) => {
              if (error) {
                logger.error(error)
              }
              return next()
    ...
    this.authorize = function (user, resource, action, isForAttachments, callback) {
      if (!resource) {
        return callback()
      }
      user = user || {}
      ...
      groups.forEach(groupName => {
        return q.defer(next => {
          self.groups.find({ name: groupName }, (error, group) => {
            ...
            return self.groups.update(group._id, group, (error) => {
              if (error) {
                return next(error)
              }
              return next()
    ...
    this.bootstrapFunctions.push(function (callback) {
      const q = queue(1)
      q.defer(next => cleanRecord(groups, {name: 'admins'}, next))
      q.defer(next => cleanRecord(groups, {name: 'anonymous'}, next))
      q.defer(next => cleanRecord(users, {username: 'localAdmin'}, next))
      q.defer(next => {
        ...
        return groups.find({ name: 'admins' }, (error, result) => {
          if (error && (error.code === 404)) {
            return groups.create({
              ...
              attachments: cms._resourceNames,
      ...
      q.defer(next =>
        ...
        users.find({ username: 'localAdmin' }, (error) => {
          if (error && (error.code === 404)) {
            ...
            return users.create(localAdminData, (error) => {
              if (error) {
                logger.error(error)
              }
              next()
    ...
    this.dispatchAuth = async (req, res, next) => {
      if (!_.get(options, 'disableJwtLogin', false)) {
        ...
        await verifyBasicAuth(req, res, next)
      } else {
        next()
      }
    }
