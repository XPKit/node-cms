...
const Authentication = options => {
  ...
  return function () {
    ...
    const verifyToken = this.verifyToken = (req, res, next = false) => {
      const token = req.body.token || req.query.token || req.headers['x-access-token'] || _.get(req, 'cookies.nodeCmsJwt', false)
      if (!token) {
        throw new Error('A token is required for authentication')
      }
      try {
        logger.warn(`verifyToken`)
    ...
    const authenticate = this.authenticate = async (username, password, req, res) => {
      let ip
      ...
      if (_.get(options, 'auth.secret', false)) {
        user.token = jwt.sign(user, options.auth.secret, {expiresIn: '24h'})
      } else {
        throw new Error(`No auth.secret in config`)
      }
      if (res) {
        res.cookie('nodeCmsJwt', user.token, { maxAge: 60 * 60 * 24 * 1000 })
      }
      return {
        result: user
      }
    ...
    // ApiManager
    const injectUser = this.injectUser = async function (req, res, next) {
      try {
        if (!_.get(req, 'session', false)) {
          throw new Error('session not found')
        }
        if (_.isEmpty(req.session.nodeCmsJwt)) {
          throw new Error('jwt not found')
        }
        if (_.isEmpty(req.session.nodeCmsUser)) {
          throw new Error('user not found')
        }
        if (_.isEmpty(req.session.nodeCmsUser.group)) {
          throw new Error('user group not found')
        }
        if (!_.isNumber(req.session.nodeCmsUser.expiredAt)) {
          throw new Error('expiredAt is not a number')
        }
        if (req.session.nodeCmsUser.expiredAt < Date.now()) {
    ...
    // LoginManager
    this.onGetUser = async function (req, res, next) {
      logger.debug(`onGetUser`, _.get(req, 'session.nodeCmsUser', {}))

      // res.send(_.get(req, 'session.nodeCmsUser', {}))
      next()
    }

    this.onGetLogout = async function (req, res, next) {
      try {
        logger.debug(`onGetLogout`)
    ...
    this.onPostLogin = async (req, res) => {
      try {
        ...
        const user = await users.find({id: req.session.nodeCmsUser.id})
        if (user && user.group) {
      ...
      // TODO: hugo - check if we need in a different var since it's already in req.session.nodeCmsUser.token
      _.set(req, 'session.nodeCmsJwt', _.get(req, 'query.jwt', false))
      // logger.debug(`onPostLogin - will return user`, userToReturn)
      res.status(200).send(userToReturn)
    }

    this.authUser = async function (req, res, next) {
    // TODO: hugo - to test
      const {username, password} = _.get(req, 'body', {})
      if (username && password) {
        logger.warn(`authAuth, received username ${username}, will try to login`)
        const {error, result} = await authenticate(username, password, req, res)
        if (error) {
      ...
      logger.debug(`authUser - user is:`, _.get(req, 'session.nodeCmsUser', false))
      next()
    }

    // TODO: hugo - test the routes in the unit tests
    const app = express()
    ...
    app.use([
      '/api/_syslog',
      '/api/system',
      '/sync/:env(local|remote)/:resource([\\w-_]+)',
      '/sync/:resource([\\w-_]+)/from/:from(local|remote)/to/:to(local|remote)',
      '/admin/resources',
      '/resources'
    ], injectUser, verifyToken)
    // app.use('/admin/resources', this.authUser)
