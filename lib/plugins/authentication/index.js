  ...
  return function () {
    ...
    this.getUserGroupName = async (req) => {
      ...
      try {
        ...
        // console.warn(`SET GROUP !`, _.get(req, 'session.nodeCmsUser.group', false))
    ...
    const verifyBasicAuth = this.verifyBasicAuth = (req, res, next = false) => {
      console.warn(`verifyBasicAuth - ${req.originalUrl}`)
    ...
    const verifyToken = this.verifyToken = (req, res, next = false) => {
      try {
        console.warn(`verifyToken - ${req.originalUrl}`)
        try {
    ...
    this.authenticateJwt = async (username, password, req, res) => {
      await this.authenticate(username, password, req, res, true)
    }
    const authenticate = this.authenticate = async (username, password, req, res, isJwt = false) => {
      let ip
    ...
    if (jwtLogin) {
      app.use(routesToAuth, verifyToken)
      ...
      app.use(routesToAuth, verifyBasicAuth)
    }
    this._app.use('/', app)
  }
}
