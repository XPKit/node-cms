...
const Authentication = options => {
  ...
  return function () {
    ...
    const verifyToken = this.verifyToken = (req, res, next) => {
      const token = req.body.token || req.query.token || req.headers['x-access-token']
      if (!token) {
        throw new Error('A token is required for authentication')
      }
      try {
        console.warn(`will verify jwt`, options.auth.secret)

        const decoded = jwt.verify(token, options.auth.secret)
        _.set(req, 'session.nodeCmsUser', decoded)
        console.warn(`verifyToken = `)
      } catch (err) {
    ...
    const authenticate = this.authenticate = async (username, password, req) => {
      let ip
      ...
      // console.warn('authenticate = user with token is', user)
      // verifyToken(req)
      return {
        result: user
      }
    ...
    this.onPostLogin = async (req, res) => {
      try {
        ...
        const {error, result} = await authenticate(username, password, req)
        if (error) {
