...
class Authentication {
  constructor(cms, options, configPath) {
    ...
    this.options = options || cms?.options || {}
  ...
  getUserGroupName = async (req) => {
    ...
    const group = _.get(nodeCmsUser, 'group.name', false)
    if (group) {
      return group
    }
  ...
  getTokenFromReq = (req) => {
    return (
      ...
      _.get(req, 'session.nodeCmsJwt') ||
      _.get(req, 'cookies.nodeCmsJwt', false)
    )
  }
  ...
  authenticate = async (username, password, req, res, isJwt = false) => {
    ...
    if (this.options.blockRetry) {
      ...
      if (blockRecord?.blockUntil && blockRecord.blockUntil > Date.now()) {
    ...
    if (hash !== user.password) {
      if (this.options.blockRetry) {
        this.blockRecordMap[username] = this.blockRecordMap[username] || {}
        this.blockRecordMap[username][ip] = this.blockRecordMap[username][ip] || { count: 0 }
        const blockRecord = this.blockRecordMap[username][ip]
  ...
  initialize = () => {
    ...
    this.cms.bootstrapFunctions.push(async (callback) => {
      ...
      try {
        const localAdmin = await this.users.find({ username: 'localAdmin' })
