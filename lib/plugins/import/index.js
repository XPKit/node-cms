  ...
  async downloadDataFromGsheetList (gsheetList) {
    ...
    await pAll(_.map(gsheetList, sheet => {
      return async () => {
        ...
        if (jsonLastModified) {
          gsheetLastModified = await this.getGsheetLastModifiedDate(this.config.gsheetId, jwtClient)
      ...
      return async () => {
        ...
        _.each(rows, row => {
          ...
          if (uniqueKeys.length === 1) {
            ...
            if (hasSubKeys && _.isFunction(hasSubKeys) && !_.isEmpty(subKeys)) {
...
const ImportPlugin = function (options) {
  return function () {
    const plugin = new ImportClass(options.import, this.api(), this)
    this._app.use('/import', plugin.express())
  }
}
...
exports = (module.exports = ImportPlugin)
