...
class ImportClass {
  constructor(cms, options) {
    ...
    this._app = cms._app
    this.config = _.get(options, 'import', options)
    this.api = cms.api()
    this.schemaMap = {}
    this.data = {}
  ...
  initialize = () => {
    ...
    app.post('/statusXlsx', upload.single('xlsx'), this.onGetStatusXlsx)
    app.post('/executeXlsx', upload.single('xlsx'), this.onGetExecuteXlsx)
    _.each(_.keys(this.cms._resources), (item) => {
      const options = _.get(this.api(item), '_resource.options', {})
    ...
    _.each(importResources, (resource) => {
      this.cms._resources[resource].import = async () => {
        ...
        await pAll(
          _.map(importMap, (map, resource) => {
            return async () => {
              await this.addImportFunctions(map, resource)
            }
          }),
          { concurrency: 1 },
        )
    ...
    this._app.use('/import', app)
  }

  addImportFunctions = async (map, resource) => {
    if (!_.isEmpty(map.remove)) {
      await pAll(
        _.map(map.remove, (item) => {
  ...
  onGetStatus = async (_req, res, next) => {
    try {
      const status = {}
      ...
      console.error('Error in onGetStatus:', error?.stack ? error.stack : error)
      next(error)
    }
  }

  onGetExecute = async (_req, res, next) => {
    try {
      const status = {}
      await this.loadDataFromSpreadSheet()
      ...
      logger.error('Error in onGetExecute:', error?.stack ? error.stack : error)
      next(error)
    }
  }
  ...
  async onGetStatusXlsx(req, res, next) {
    try {
      ...
      logger.error('Error in onGetStatusXlsx:', error?.stack ? error.stack : error)
      next(error)
    }
  }
  ...
  async onGetExecuteXlsx(req, res, next) {
    try {
      ...
      console.error('Error in onGetExecuteXlsx:', error?.stack ? error.stack : error)
      next(error)
    }
  }
  ...
  async getImportMap(createOnly, resourceFilter) {
    ...
    await pAll(
      _.map(filteredData, (importList, key) => {
        return async () => {
  ...
  async getGsheetList(resource) {
    ...
    gsheetList = _.compact(
      _.map(gsheetList, (key) => {
        return { id: key }
      }),
    )
    endProcess('done')
  ...
  readKeyFile() {
    return fs.readFileSync(this.config.oauth.keyFile, 'utf-8')
  }
    ...
    await pAll(
      _.map(gsheetList, (sheet) => {
        return async () => {
          ...
          const rows = {}
          _.each(
            _.reject(_.keys(ws), (item) => _.startsWith(item, '!')),
            (key) => {
    ...
    return await pAll(
      _.map(gsheetList, (sheet) => {
        return async () => {
          ...
          const jsonPath = path.join(this.cachedFolder, 'cached', `${sheet.id}-active.json`)
