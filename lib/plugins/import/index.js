...
class ImportClass {
  ...
  onGetStatus = async (req, res, next) => {
    try {
      const status = {}
      ...
      console.error('Error in onGetStatus:', error && error.stack ? error.stack : error)
      next(error)
    }
  }

  onGetExecute = async (req, res, next) => {
    try {
      const status = {}
      await this.loadDataFromSpreadSheet()
      ...
      logger.error('Error in onGetExecute:', error && error.stack ? error.stack : error)
      next(error)
    }
  }

  onGetStatusXlsx = async (req, res, next) => {
    try {
      const status = {}
      ...
      logger.error('Error in onGetStatusXlsx:', error && error.stack ? error.stack : error)
      next(error)
    }
  }

  onGetExecuteXlsx = async (req, res, next) => {
    try {
      const status = {}
      await this.loadDataFromXlsx(req.file)
      const keys = _.keys(this.data)
      ...
      console.error('Error in onGetExecuteXlsx:', error && error.stack ? error.stack : error)
      next(error)
    }
  }
