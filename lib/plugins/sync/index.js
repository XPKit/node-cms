...
class SyncClass {
  ...
  initialize() {
    ...
    this.app.post(
      '/:resource([\\w-_]+)/from/:from(local|remote)/to/:to(local|remote)',
      this.checkResource,
      this.onPostSyncResourceFromTo,
    )
    this.app.get(
      '/:resource([\\w-_]+)/from/:from(local|remote)/to/:to(local|remote)',
      this.checkToken,
  ...
  async getSyncResourceItems(req, resource) {
    ...
    items = newItems
    items = _.map(items, (item) => _.omitBy(item, (_value, key) => _.startsWith(key, '_') && key !== '_attachments'))
    if (resource === 'markets') {
      items = _.map(items, (item) => {
  ...
  async checkEnvironmentToken(req, _res, next) {
    try {
      const syncConfig = await this.api('_sync').find({})
  ...
  async checkResource(req, _res, next) {
    try {
      const resource = req.params.resource
  ...
  async getRelationMap(resource) {
    ...
    await pAll(
      _.map(relationResources, (resource) => {
        return async () => {
  ...
  async startSyncData(req, resource, options) {
    try {
      ...
      const createKeys = _.filter(fromKeys, (query) => !_.find(toKeys, query))
      ...
      await pAll(
        _.map(createAndUpdateItems, (item) => {
          return async () => {
            ...
            if (localItem) {
              ...
              const keys = _.keys(pickItem)
              const pickedLocalItem = _.pick(localItem, keys)
              if (resource === 'markets') {
            ...
            // remove all
            await pAll(
              _.map(remoteAttachments, (remoteList, name) => {
                return async () => {
