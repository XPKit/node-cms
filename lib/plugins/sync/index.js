...
class SyncClass {
  ...
  async init () {
    ...
    this.app.get('/:env(local|remote)/:resource([\\w-_]+)/status', this.authenticate, this.checkEnvironmentToken, this.checkResource, this.onGetSyncEnvironmentResourceStatus)
    this.app.get('/:env(local|remote)/:resource([\\w-_]+)', this.authenticate, this.checkEnvironmentToken, this.checkResource, this.onGetSyncEnvironmentResource)
    this.app.post('/:resource([\\w-_]+)/from/:from(local|remote)/to/:to(local|remote)', this.authenticate, this.checkResource, this.onPostSyncResourceFromTo)
    ...
    this.app.get('/:resource([\\w-_]+)', this.checkToken, this.checkResource, this.onGetSyncResource)
    this.app.get('/:resource([\\w-_]+)/status', this.checkToken, this.checkResource, this.onGetSyncResourceStatus)
    this.app.put('/:resource([\\w-_]+)', this.checkToken, this.checkResource, this.onPutSyncResource)
    this.app.get('/:resource([\\w-_]+)/:id([\\w-_]+)/attachments/:aid([\\w-_]+)', this.checkToken, this.checkResource, this.onGetSyncResourceAttachment)
  ...
  async onPostSyncResourceFromTo (req, res, next) {
    try {
      ...
      try {
        await got.put(url, {json: items}).json()
      } catch (error) {
        throw _.get(error, 'response.body.error', error)
      }
  ...
  async onGetSyncEnvironmentResourceStatus (req, res, next) {
    try {
      let result = await got.get(`${req.syncInfo.url}/sync/${req.params.resource}/status?token=${req.syncInfo.token}`).json()
      res.send(result)
    } catch (error) {
      next(error)
    }
  ...
  async onGetSyncEnvironmentResource (req, res, next) {
    try {
      let result = await got.get(`${req.syncInfo.url}/sync/${req.params.resource}?token=${req.syncInfo.token}`).json()
      res.send(result)
    } catch (error) {
      next(error)
    }
            ...
            return async () => {
              ...
              if (_.map(localList, '_md5sum').join(',') !== _.map(remoteList, '_md5sum').join(',')) {
                ...
                await pAll(_.map(remoteList, attach => {
                  return async () => {
                    try {
                      const response = await got.get(attach.url, { responseType: 'buffer' })
                      var file = new stream.Readable()
                      file.push(response.rawBody)
                      file.push(null)
