...
let xpkitConfig = false
try {
  xpkitConfig = _.get(require(path.join(process.cwd(), 'cms.json')), 'xptoolkit', false)
} catch (error) {
  logger.error(`Couldn't get xpkitConfiguration.json:`, error)
}
...
XpkitIterator.prototype._next = async function (callback) {
  try {
    // console.warn(`ITERATOR - _NEXT --------------------- `, this.cursor)
    ...
    console.warn(`Error on next ---`, error)
...
function XpkitDown (location) {
  ...
  this.XpkitClient = new XpkitClient(xpkitConfig)
...
XpkitDown.prototype._open = async function (options, callback) {
  try {
    this.collection = new XpkitCollection(this.collectionName, xpkitConfig)
  } catch (error) {
...
XpkitDown.prototype._get = async function (key, options, callback) {
  try {
    // console.warn(`XpkitDown - GET`, key, options)
    let record = await this.collection.findOne({_id: key})
    if (!record) {
      callback(new Error('NotFound'))
    } else {
