...
function MongoDOWN (location) {
  ...
  try {
    ...
    console.error(`unable to json parse indexMap string ${indexMap}`)
  }
}
...
MongoDOWN.prototype.getClient = async function (host) {
  ...
  // If a promise for this host already exists, wait for it
  if (gClientPromiseMap[host]) {
    return gClientPromiseMap[host]
  }

  // Create a new promise for this host
  gClientPromiseMap[host] = (async () => {
    const params = {}
...
MongoDOWN.prototype._iterator = function (options) {
  let cursor = this.collection.find(options.query).sort({ _createdAt: 1 }).allowDiskUse(true)
