...
class JsonDOWN extends AbstractLevel {
  constructor(location, options = {}) {
    ...
    this._prewriteHooks = []
    // Track closing state and pending writes
    this._closing = false
  ...
  async _persist() {
    ...
    if (this._closing) {
      // If closing, do not persist further writes
      return
    }
    this._pendingWrites++
    try {
      ...
      this._pendingWrites--
      if (this._closing && this._pendingWrites === 0) {
        // Resolve all close waiters
        for (const resolve of this._pendingWriteResolvers) {
          resolve()
        }
        this._pendingWriteResolvers = []
      }
  ...
  async _batch(operations, _options) {
    ...
    // After this point, no new writes will be allowed
    console.warn('JsonDOWN: All writes stopped, DB closed.')
