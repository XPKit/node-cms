...
function sortedIndexOf (arr, item) {
  let low = 0; let high = arr.length; let mid
  while (low < high) {
    mid = (low + high) >>> 1
...
function MemIterator (db, options) {
  ...
  this._keys = []
  // TODO: hugo - check how to order results by _updatedAt desc
  let i
  ...
  if (!options.start || this._pos === undefined) { this._pos = this._reverse ? this.db._keys.length - 1 : 0 }
...
MemIterator.prototype._next = function (callback) {
  ...
  if (self._pos >= self._keys.length || self._pos < 0) { return setImmediate(callback) }

  if (!!self._end && (self._reverse ? key < self._end : key > self._end)) { return setImmediate(callback) }

  if (!!self._limit && self._limit > 0 && self._count++ >= self._limit) { return setImmediate(callback) }

  if ((this._lt && key >= this._lt) ||
     (this._lte && key > this._lte) ||
     (this._gt && key <= this._gt) ||
     (this._gte && key < this._gte)) { return setImmediate(callback) }
