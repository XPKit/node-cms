...
class JsonStore {
  ...
  async read (query, _options) {
    ...
    // here and applied afterwards by the filterResults hook, which pages the matches itself. Cutting
    // the read short would leave that hook filtering an arbitrary slice of the collection, so the
    // limit is only honoured when nothing is left to filter. The mongodb and postgres stores build
    // the query into their own read and have already filtered by this point.
    const filteredAfterwards = this._dbType === 'default' && !!query && _.isObject(query) && !_.isEmpty(query)
    const limit = filteredAfterwards ? 0 : _.get(_options, 'limit', 0)
