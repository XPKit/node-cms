...
class Resource {
  ...
  constructor(name, options, resolveMap, cms) {
    ...
    this.options.cms.gJStoreMap[key] =
      this.options.cms.gJStoreMap[key] || new JStore(jpath, this.options.cms.mid, this.options, name)
    this.json = this.options.cms.gJStoreMap[key]
    this.options.cms.gFStoreMap[key] =
      this.options.cms.gFStoreMap[key] || new FStore(apath, this.options.cms.mid, this.options)
    this.file = this.options.cms.gFStoreMap[key]
    ...
    this._openDbPromise = this.json.open().then(() => {
      this._dbOpened = true
    })
    this.initDriver()
  }
   ...
   */
  convertKeyToId(value, type, records, _name, uniqueKeys, errors) {
                ...
                _.each(field.locales, (locale) => {
                  ...
                  if (isCheckRequired && field.required && (_.isUndefined(v) || (!_.isArray(v) && _.isEmpty(v)))) {
                    throw new RequiredFieldError(
                      `record (${JSON.stringify(keyObj)}), required field (${field.field}) and value (${oldKey}) is invalid`,
                    )
                  }
  ...
  async handleReplication(context) {
    const shouldReplicate =
      _.isFunction(_.get(this.cms, '$replicator.syncRecord', false)) && this.options.type !== 'normal'
    if (shouldReplicate) {
      try {
  ...
  setupDriverHooks() {
    ...
    driver.before('remove', async (context) => {
      try {
        await pAll(
          _.map(context.record._attachments, (attachment) => {
            return async () => {
              try {
  ...
  setupAttachmentHooks(driver) {
    ...
    driver.before('createAttachment', async (context) => {
      if (_.get(context, 'params.object.name.length', 0) === 0) {
        return context.error(
          h.removeAttachmentError(context.resource.name, context.record, null, new Error('missing field name')),
        )
      }
      ...
      if (_.includes(imageTypes, contentType)) {
        try {
          ...
          if (_.get(obj, 'smart', false)) {
            ...
            const result = await ImageOptimization.smartCropAttachment(streamWithFileType, resize, smartCropOptions)
            ...
            if (resize && result.targetSize && result.targetSize.height && result.targetSize.width) {
              ...
              if (
                (enforceWidth && result.targetSize.width !== enforceWidth) ||
                (enforceHeight && result.targetSize.height !== enforceHeight)
              ) {
                // Re-resize to enforce output dimensions
                const { stream: resizedStream } = await ImageOptimization.resizeAttachment(
                  finalStream,
                  resize,
                  context.params.object.contentType,
                )
            ...
            if (resizeTo) {
              const { stream } = await ImageOptimization.resizeAttachment(
                streamWithFileType,
                resizeTo,
                context.params.object.contentType,
              )
    ...
    driver.before('createAttachment', (context) => {
      const obj = context.params.object
      context.attachmentId = context.resource.options.cms.uuid()
      ...
      if (!_.isFunction(_.get(obj, 'stream.on', false))) {
        return context.error(
          h.createAttachmentError(context.resource.name, context.params.id, new Error('Invalid stream object')),
        )
      }
    ...
    driver.before('removeAttachment', async (context) => {
      try {
        ...
        if (error.notFound) {
          return context.error(
            h.findAttachmentError(context.resource.name, context.params.id, context.params.aid, error),
          )
        }
        return context.error(
          h.removeAttachmentError(context.resource.name, context.params.id, context.params.aid, error),
        )
      }
    })
  ...
  async list(query, options) {
    ...
    const results = await this.read(query, queryOptions)
  ...
  async remove(id) {
    ...
    if (record._attachments && record._attachments.length > 0) {
      await pAll(
        _.map(record._attachments, (attachment) => {
          return async () => {
            try {
              try {
                ...
                logger.error(
                  `Couldn't remove default resized attachment ${attachment._id}-${this.defaultResize}`,
                  error,
                )
              }
