...
import JStore from './db/json_store.js'
import FStore from './db/file_store.js'
import createDriver, { Driver } from './util/driver/index.js'
import ImageOptimization from './util/imageOptimization.js'
...
class Resource {
  ...
  initDriver () {
    ...
    this._driver = createDriver(this.name, this.options)
  ...
  setupDriverHooks() {
    ...
    driver.define('read', ['query?', 'options?'], Driver._readImpl)
    driver.after('read', h.filterResults)

    // list hooks
    driver.before('list', h.getDependencyItems.bind(this))
    driver.define('list', ['query?', 'options?'], Driver._listImpl)

    // find hooks
    driver.before('find', h.getDependencyItems.bind(this))
    driver.define('find', ['query', 'options?'], Driver._findImpl)

    // exists
    driver.define('exists', ['query'], Driver._existsImpl)
    ...
    driver.define('create', ['object', 'options?'], Driver._createImpl)
    ...
    driver.define('update', ['id', 'object', 'options?'], Driver._updateImpl)
        ...
        await pAll(_.map(context.record._attachments, attachment => {
          return async () => {
            try {
              ...
              await Driver.cleanAttachmentCache(context.resource, attachment._id)
    ...
    driver.define('remove', ['id'], Driver._removeImpl)
  ...
  setupAttachmentHooks(driver) {
      ...
      if (_.includes(imageTypes, contentType)) {
        try {
          ...
          if (field && field.options && field.options.width && field.options.height) {
            const {stream} = await ImageOptimization.resizeAttachment(streamWithFileType, `${field.options.width}x${field.options.height}`, context.params.object.contentType)
    ...
    driver.before('createAttachment', (context) => {
      ...
      if (!obj.stream || typeof obj.stream.on !== 'function') {
    ...
    driver.define('createAttachment', ['id', 'object'], Driver._createAttachmentImpl)
    ...
    driver.before('updateAttachment', async context => {
      try {
        ...
        await Driver.cleanAttachmentCache(context.resource, context.params.aid)
        context.next()
      } catch (error) {
    ...
    driver.define('updateAttachment', ['id','aid','object'], Driver._updateAttachmentImpl)
    ...
    // findFile
    driver.define('findFile', ['aid'], Driver._findFileImpl)

    // findAttachment hooks
    driver.before('findAttachment', h.findRecord)
    driver.before('findAttachment', h.findRecordAttachment)
    driver.define('findAttachment', ['id', 'aid', 'opts?'], Driver._findAttachmentImpl)
    ...
    driver.before('removeAttachment', h.findRecord)
    driver.define('removeAttachment', ['id', 'aid'], Driver._removeAttachmentImpl)
    ...
    // cleanAttachment
    driver.define('cleanAttachment', [], Driver._cleanAttachmentImpl)
  }
  ...
  async list(query, options) {
    ...
    //     .then(gotStream => {
    //       console.warn('LIST IMPL: entering Resource.list stream pipeline')
    //       const results = []
    //       gotStream.pipe(through(
    //         data => {
    //           console.warn('LIST IMPL 2: record entering Resource.list', data)
