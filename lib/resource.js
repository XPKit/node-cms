...
class Resource {
  ...
  setupAttachmentHooks(driver) {
    ...
    driver.define('findAttachment', ['id', 'aid'], Driver.Driver._findAttachmentImpl)
  ...
  async find(query, options) {
    ...
    if (_.isString(query)) {
      try {
        ...
        if (error.notFound) {
          throw h.recordNotFoundErrorV2(this.name, query, error)
        }
        throw h.recordNotFoundErrorV2(this.name, query, error)
      }
    }
    ...
    try {
      ...
      if (!results.length) {
        throw h.listNotFoundErrorV2(this.name, query)
      }
      return results[0]
    } catch (error) {
      throw h.listRecordErrorV2(this.name, query, error)
    }
  }
      ...
      await pAll(_.map(record._attachments, attachment => {
        return async () => {
          try {
            ...
            throw h.removeAttachmentErrorV2(this.name, record._id, attachment._id, error)
          }
        }
      }), {concurrency: 10})
    }
