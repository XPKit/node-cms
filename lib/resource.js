...
class Resource {
  ...
  initDriver () {
    ...
    driver.before('createAttachment', async (context) => {
      ...
      if (!_.get(obj, 'contentType', false)) {
        logger.info('No contentType provided for attachment, will try to determine it')
        obj.contentType = _.get(await FileType.getBufferFromStream(obj.stream), 'mime', 'application/octet-stream')
      }
      if (!_.get(obj, 'filename', false)) {
