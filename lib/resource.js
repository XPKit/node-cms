...
class Resource {
  ...
  initDriver () {
    ...
    driver.before('createAttachment', async (context) => {
      ...
      context.params.object.stream.pipe(mimeStream)
      mimeStream.on('type', async (type)=> {
        context.params.object.contentType = _.get(type, 'mime', 'application/octet-stream')
        console.log(context.params.object.contentType)
        if (_.includes(['image/jpeg', 'image/gif', 'image/png', 'image/svg+xml'], context.params.object.contentType)) {
