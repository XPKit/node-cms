...
class Resource {
  ...
  initDriver () {
    ...
    driver.define('read', ['query?', 'fields?', 'options?'], context => {
      console.warn('read fields ', context.params.fields)
    ...
    driver.define('list', ['query?', 'fields?', 'options?'], (context) => {
      context.params.options = context.params.options || {}
      console.warn('LIST FIELDS - ', context.params.fields)
    ...
    driver.define('find', ['query', 'options?'], (context) => {
    ...
    driver.define('create', ['object', 'options?'], context => context.resource.json.create(context.params.object._id, context.params.object, (error, result) => {
      if (error) {
    ...
    driver.define('update', ['id', 'object', 'options?'], (context) => {
      ...
      try {
        await pAll(_.map(context.record._attachments, attachment => {
          return () => Q.ninvoke(context.resource.file, 'remove', attachment._id)
            .catch(error => { throw h.removeAttachmentError(context.resource.name, context.record._id, attachment._id, error) })
        }), {concurrency: 10})
        context.next()
      } catch (error) {
        context.error(error)
    ...
    driver.before('createAttachment', async (context) => {
      ...
      if (_.get(obj, 'noOptimization', false) === false) {
        if (field && field.options && field.options.width && field.options.height) {
          if (_.includes(['image/png', 'image/jpeg', 'image/gif', 'image/svg'], obj.contentType)) {
            console.warn('Will resize attachment and optimize it', obj.contentType)
            const {stream} = await ImageMin.resizeAttachment(obj.stream, field.option)
            obj.stream = obj.stream.pipe(stream)
          }
        }
      }
      context.next()
    ...
    driver.before('createAttachment', async (context) => {
      try {
        ...
        if (_.get(field, 'input') === 'image') {
          ...
          if (maxCount === 1) {
            // console.warn('old attachments = ', field, oldAttachments)
            // logger.warn('maxCount === 1, will delete previous attachments')
            ...
            await pAll(_.map(oldAttachments, attach => {
              return async () => await Q.ninvoke(context.resource.file, 'remove', attach._id)
            }), {concurrency: 1})
    ...
    driver.before('removeAttachment', h.findRecord)
    driver.define('removeAttachment', ['id', 'aid'], (context) => {
