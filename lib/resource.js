...
class Resource {
  ...
  async handleReplication(context) {
    const shouldReplicate = _.isFunction(_.get(this.cms, '$replicator.syncRecord', false)) && this.options.type !== 'normal'
    if (shouldReplicate) {
      try {
        await this.cms.$replicator.syncRecord(context.options.name, _.get(context, '_result._id'))
      } catch (err) {
        console.error('Replicator syncRecord after create failed:', err)
      }
    }
  }
  ...
  setupAttachmentHooks(driver) {
    ...
    driver.before('createAttachment', async (context) => {
      ...
      if (_.includes(imageTypes, contentType)) {
        try {
          const type = await ImageOptimization.getFileTypeFromFile(obj.stream.path)
          if (type) {
          ...
          if (_.get(obj, 'smart', false)) {
            // Use smartCropOptions from object
            const smartCropOptions = {
              resize,
              smart: _.get(obj, 'smart', false),
              objectDetection: _.get(obj, 'objectDetection', false),
              faceOnly: _.get(obj, 'faceOnly', false),
              facePadding: _.get(obj, 'facePadding', 10)
            }
            // Always enforce requested output dimensions after smart crop
            const result = await ImageOptimization.smartCropAttachment(obj.stream, resize, smartCropOptions)
            // If the result is not the requested size, resize again
            let finalStream = result.stream
            if (resize && result.targetSize && result.targetSize.height && result.targetSize.width) {
              const [targetWidth, targetHeight] = resize.split('x')
              const enforceWidth = targetWidth !== 'auto' ? Number(targetWidth) : null
              const enforceHeight = targetHeight !== 'auto' ? Number(targetHeight) : null
              if ((enforceWidth && result.targetSize.width !== enforceWidth) || (enforceHeight && result.targetSize.height !== enforceHeight)) {
                // Re-resize to enforce output dimensions
                const {stream: resizedStream} = await ImageOptimization.resizeAttachment(finalStream, resize, context.params.object.contentType)
                finalStream = resizedStream
              }
            }
            context.params.object.stream = finalStream
          } else {
            let resizeTo = false
            ...
            if (resizeTo) {
              const {stream} = await ImageOptimization.resizeAttachment(obj.stream, resizeTo, context.params.object.contentType)
    ...
    driver.after('createAttachment', async (context) => {
      ...
      // console.warn('Created attachment:', context)
      context.next()
    })
    ...
    driver.before('updateAttachment', async context => {
      try {
        ...
        console.error('Failed to get image dimensions for update:', error)
        context.next()
      }
    })
    ...
    driver.before('removeAttachment', async context => {
      try {
        ...
        if (h.isOSSField(field)) {
          ...
          // Clean all cached versions of this attachment
          await Driver.Driver.cleanAttachmentCache(context.resource, context.params.aid)
          context.resource.file.remove(context.params.aid)
        }
        return context.next()
      } catch (error) {
        if (error.notFound) {
  ...
  async read(query, options) {
    const results = await this.json.read(query, options)
    _.each(results, (record)=> {
      record._local = h.isOwnRecord(_.get(record, '_id', ''), this.options.cms.mid)
    })
    // Create a readable stream from the results array
    let index = 0
    const stream = new Readable({
      objectMode: true,
      read() {
        if (index < results.length) {
          this.push(results[index++])
        } else {
  ...
  async list(query, options) {
    // Ensure DB is open before listing
    if (!this._dbOpened) {
      await this._openDbPromise
    }
    // Get dependency items first
    const dependencyMap = await h.getDependencyItems.call(this, query, options)
    // Setup options
    const queryOptions = options || {}
    let results = await this.read(query, queryOptions)
    return _.map(results, (result)=> h.injectDependency(result, dependencyMap, this.options, this.resolveMap))
  }
  ...
  async find(query, options) {
    ...
    // Ensure DB is open before finding
    if (!this._dbOpened) {
      await this._openDbPromise
    }
    // Get dependency items first
    ...
    if (_.isString(query)) {
      try {
        ...
        result._local = h.isOwnRecord(_.get(result, '_id', ''), _options.cms.mid)
   ...
   */
  async exists(query) {
    // Ensure DB is open before exists check
    if (!this._dbOpened) {
      await this._openDbPromise
    }
    if (_.isString(query)) {
  ...
  async create(object, options) {
    const _options = this.options
    // Ensure DB is open before create
    if (!this._dbOpened) {
      await this._openDbPromise
    }
    // Apply normalization
    ...
    await h.checkUniqueFields(context)
  ...
  async update(id, object, _options) {
    // Ensure DB is open before update
    if (!this._dbOpened) {
      await this._openDbPromise
    }
    // Before the merge below, which mutates the record the store handed back: on the default
    // store that is the stored object itself, so a refused update could still be observed by a
    // later read (#105).
    if (!this.json.owns(id)) {
      throw h.foreignRecordError(this.name, id)
    }
   ...
   */
  async remove(id) {
    // Ensure DB is open before remove
    if (!this._dbOpened) {
      await this._openDbPromise
    }
    // Refused before anything is touched. The store refuses a foreign record too, but by then the
    // attachments below have already been deleted - a refused delete that destroys the blobs and
    // leaves the record is worse than either outcome (#105).
    if (!this.json.owns(id)) {
      throw h.foreignRecordError(this.name, id)
    }
    ...
    logger.warn(`Removing record ${id} with ${_.get(record, '_attachments.length', 0)} attachments`)
    if (record._attachments && record._attachments.length > 0) {
      await pAll(_.map(record._attachments, attachment => {
        return async () => {
          try {
            // Find the schema field for this attachment
            const field = _.find(this.options.schema, { field: attachment._name })
            if (h.isOSSField(field)) {
              const _filename = attachment._filename || _.get(attachment, '_fields._filename', '') || _.get(attachment, 'filename', '') || _.get(attachment, 'fields._filename', '')
              // Build a context for OSSHelper
              const context = {
                resource: this,
              ...
              logger.info(`Deleting OSS file for record ${record._id}, attachment ${attachment._id}, filename: ${_filename}`)
              await this.cms.oss.deleteFile(context, field)
            } else {
              try {
              ...
              await this.file.remove(attachment._id)
            }
          } catch (error) {
