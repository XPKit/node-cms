...
class Resource {
  ...
  cleanDataBasedOnSchema(schema, data) {
    const resourceName = _.get(schema, 'displayname.enUS', _.get(schema, 'displayname', 'not-found'))
    const formattedData = {}
    _.each(data, (field, key) => {
      const foundField = _.find(schema, { field: key })
      if (!_.includes(['_updatedBy', '_type'], key) && _.isUndefined(foundField)) {
        return logger.error(`Field ${key} doesn't exist in schema for ${resourceName}, will remove it`)
      }
      const fieldInput = _.get(foundField, 'input', false)
      if (!fieldInput) {
        return logger.error(`Field ${key} doesn't have an 'input' in the schema, will remove it`)
      }
      if (fieldInput !== 'paragraph') {
        _.set(formattedData, key, field)
        return
      }
      _.each(field, (paragraphItem, i)=> {
        const type = _.get(paragraphItem, '_type', false)
        if (!type) {
          return logger.warn('Type of paragraph item not specified, will remove it', paragraphItem)
        }
        _.set(formattedData, `${key}[${i}]`, this.cleanDataBasedOnSchema(this.getParagraphSchema(type), paragraphItem))
      })
    })
  ...
  cleanData(context) {
    // context.params.object = this.cleanDataBasedOnSchema(context.resource.options.schema, context.params.object)
    context.next()
  }
  ...
  initDriver () {
    ...
    // TODO: hugo - change to clean all records on server launch, not on create/update
      ...
      if (_.includes(['image/jpeg', 'image/gif', 'image/png'], contentType)) {
        ...
        mimeStream.on('type', async (type)=> {
          ...
          if (field && field.options && field.options.width && field.options.height) {
            const {stream, mimeType} = await ImageMin.resizeAttachment(mimeStream, field.option)
            context.params.object.stream = stream
            context.params.object.contentType = mimeType
          } else {
    ...
    driver.before('createAttachment', (context) => {
      ...
      writeStream.on('finish', async ()=> {
        ...
        if (_.includes(['image/jpeg', 'image/gif', 'image/png'], contentType)) {
          ...
          logger.info('Will cache resized img', `${attachmentId}-${defaultResize}`, contentType)
          try {
            const savedFileStream = context.resource.file.read(attachmentId)
            const croppedStream = context.resource.file.write(`${attachmentId}-${defaultResize}`)
            const {stream} = await ImageMin.resizeAttachment(savedFileStream, defaultResize, contentType)
            stream.pipe(croppedStream)
          } catch (error) {
            context.resource.file.remove(`${attachmentId}-${defaultResize}`)
          }
        }
        context.next()
      })
    ...
    driver.define('createAttachment', [
      ...
      const now = Date.now()
      const obj = context.params.object
      // console.warn('will create attachment with _contentType = ', obj.contentType)
    ...
    driver.before('removeAttachment', async context => {
      try {
        ...
        try {
          await context.resource.file.remove(`${context.params.aid}-autox100`)
        } catch (error) {
          logger.error(`Couldn't remove ${context.params.aid}-autox100`, error)
        }
        return context.next()
      } catch (error) {
        if (error.notFound) {
