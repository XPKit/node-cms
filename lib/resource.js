...
class Resource {
      ...
      .forEach(([k, v]) => {
        ...
        if (v && typeof v === 'object' && !Object.keys(v).length || v === null || v === undefined) {
          if (Array.isArray(object)) {
            ...
            // TODO: hugo - check if it works with all POST/PUT of records with and without paragraphs
  ...
  cleanDataBasedOnSchema(schema, data) {
    _.each(data, (field, key) => {
      ...
      // TODO: hugo - for each field and sub field in schema, check the associated schema
      // if a field isn't in the schema, remove it
      if (_.isUndefined(foundField)) {
        // console.warn(`Field ${key} doesn't exist in schema`, schema)
      } else {
        // console.warn(`Found field ${key} in schema`)
      }
    })
    return data
  }
