...
class ImportManager {
            ...
            if (!_.isEmpty(link)) {
              funcs2.push(async () => {
                ...
                const filePath = path.resolve(
                  path.join('.', resource, _.get(item, _.first(uniqueKeys)), field, filename),
                )
                let endSubProcess
                try {
            ...
            if (field.locales) {
              _.each(field.locales, (locale) => {
                ...
                v = h.convertKeyToId(
                  v,
                  field.input,
                  this.cmsRecordsMap[field.source],
                  field.source,
                  relationUniqueKeys,
                  errors,
                )
          ...
          _.map(data, (excelObject) => {
            ...
            _.each(locales, (locale) => {
              ...
              _.each(excelObject[locale], (value, key) => {
                if (isUpdated) {
                  return
                } else if (!_.isEqual(value, cmsObject[locale]?.[key])) {
                  isUpdated = true
                }
              })
            })
  ...
  async createCmsRecordAttachments(createdRecordsMap) {
    ...
    return pAll(
      _.map(_.keys(this.attachmentMap), (resource) => {
        return async () => {
    ...
    const funcs = _.map(_.keys(this.data), (resource) => {
      return async () => {
        ...
        let cmsAttachmentList = _.compact(
          _.flatten(
            _.map(this.cmsRecordsMap[resource], (item) =>
              _.map(item._attachments, (attach) => _.extend({ recordId: item._id }, attach)),
            ),
          ),
        )
        cmsAttachmentList = _.filter(cmsAttachmentList, (item) => {
          return !_.find(list, _.extend({ _id: item.recordId }, _.pick(item, ['_name', '_md5sum', '_filename'])))
        })
...
if (config?.oauth?.keyFile) {
  config.oauth.keyFile = path.resolve(config.oauth.keyFile)
}

const [username, password] = program.args[1].split(':')
const auth = { username, password }

module.exports = new ImportManager(config, auth)
