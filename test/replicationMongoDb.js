...
const SLAVE_HTTP_PORT = 5100
const MASTER_NET_PORT = 6100
const SLAVE_NET_PORT = 7100
...
it('should remove article from master and sync to slave', async () => {
  await master.request.delete(`/api/articles/${article._id}`)
  await Q.ninvoke(slave.cms, 'replicate', 'localhost', MASTER_NET_PORT, `${MASTER_URL}/api/`, 'articles')
...
// it('should have clock synced', () => {
//   master.cms.resource('articles').json._sync.clock.should
//     .deep.equal(slave.cms.resource('articles').json._sync.clock)
// })
...
describe('Downstream & Upstream', () => {
  ...
  before(async () => {
    master.cms.resource('downstream-collection', { type: 'downstream' })
    master.cms.resource('upstream-collection', { type: 'upstream' })
    slave.cms.resource('downstream-collection', { type: 'downstream' })
    slave.cms.resource('upstream-collection', { type: 'upstream' })
    await Q.ninvoke(master.cms, 'allow', 'anonymous', ['downstream-collection', 'upstream-collection'])
    await Q.ninvoke(slave.cms, 'allow', 'anonymous', ['downstream-collection', 'upstream-collection'])
  })
  ...
  it('should only pull changes from master (downstream)', async () => {
    ...
    let slaveApi = slave.cms.api()('downstream-collection')
    let masterApi = master.cms.api()('downstream-collection')
    let result
  ...
  it('should only pull changes from slave (upstream)', async () => {
    await Q.ninvoke(slave.cms, 'replicate', 'localhost', MASTER_NET_PORT, `${MASTER_URL}/api/`, 'upstream-collection')

    let masterApi = master.cms.api()('upstream-collection')
    let slaveApi = slave.cms.api()('upstream-collection')
    let result
    ...
    const masterList = await masterApi.list()
    console.log('masterList', masterList)

    const slaveList = await slaveApi.list()
    console.log('slaveList', slaveList)
