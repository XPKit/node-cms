...
describe('Replicator Plugin API', () => {
  it('DEBUG: print /replicator/resources for all nodes', async () => {
    for (const port of [9990, 9991, 9992]) {
      const res = await request(`http://localhost:${port}`).get('/replicator/resources')
  ...
  it('should replicate a new article from normal node to downstream nodes', async () => {
    // Create an article in the normal node (Node A)
    const articleData = { string: 'Replication Test', rate: 5 }
    const createRes = await request('http://localhost:9990').post('/api/articles').send(articleData)
    ...
    for (const port of [9991, 9992]) {
      const resourcesRes = await request(`http://localhost:${port}`).get('/replicator/resources')
    ...
    for (const port of [9991, 9992]) {
      const res = await request(`http://localhost:${port}`).get(`/articles/${createdId}`)
      if (res.status !== 200) {
        console.error(
          `Downstream node ${port} GET /articles/${createdId} failed: status=${res.status}, body=`,
          res.body,
        )
      } else {
  ...
  it('should NOT replicate modifications from downstream node to normal or other downstream nodes', async () => {
    // Create an article in the normal node (Node A)
    const articleData = { string: 'Downstream Edit Test', rate: 7 }
    const createRes = await request('http://localhost:9990').post('/articles').send(articleData)
    expect(createRes.status).to.equal(200)
    const createdId = createRes.body && (createRes.body._id || createRes.body.id)
    expect(createdId).to.exist
    ...
    const editRes = await request('http://localhost:9991').put(`/articles/${createdId}`).send(downstreamEdit)
  ...
  it('should NOT replicate modifications from downstream node to upstream node', async () => {
  ...
  it('should NOT replicate modifications from upstream node to other upstream nodes', async () => {
    ...
    const editRes = await request('http://localhost:9992').put(`/articles/${createdId}`).send(upstreamEdit)
    expect(editRes.status).to.be.oneOf([200, 204])
