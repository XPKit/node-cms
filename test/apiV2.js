...
before(async () => {
  cms = new CMS({data: DB, config: CONFIG, autoload: true, apiVersion: 2})
...
describe('HTTP', () => {
  describe('Resource', () => {
    ...
    it('should not find one article using invalid _id', () => {
      return request(MASTER_URL)
        .get(`/api/articles/INVALID_ID?unpublished=true`)
        .auth('localAdmin', 'localAdmin')
        .expect(404)
        .then(({body}) => {
          body.message.should.equal('record (INVALID_ID) not found in resource (articles) error')
        })
    })
  ...
  describe('Attachment', () => {
    ...
    it('should upload an image attachment', () => {
      return request(MASTER_URL)
        .post(`/api/articles/${article._id}/attachments`)
        .attach('image', `${__dirname}/_large.png`)
        .auth('localAdmin', 'localAdmin')
        .expect(200)
        .then(({body}) => {
          attachment = body
    ...
    it('should overwrite an image attachment', async () => {
      const {body} = await request(MASTER_URL)
        .post(`/api/articles/${article._id}/attachments`)
        .attach('image', `${__dirname}/_small.jpg`)
        .auth('localAdmin', 'localAdmin')
        .expect(200)

      const attachment = body
      attachment._id.should.be.a('string')
      attachment._filename.should.equal('_small.jpg')
      attachment._contentType.should.equal('image/jpeg')
      attachment._fields.should.be.an('object')
      attachment._createdAt.should.be.a('number')
      attachment._updatedAt.should.be.a('number')
      attachment._createdAt.should.equal(attachment._updatedAt)
    ...
    it('should have 1 image attachment only', async () => {
      const {body} = await request(MASTER_URL)
        .get(`/api/articles/${article._id}`)
        .auth('localAdmin', 'localAdmin')
        .expect(200)
      _.filter(body._attachments, {_name: 'image'}).should.have.lengthOf(1)
    })

    it('should remove document with attachment', () => {
      return request(MASTER_URL)
        .post(`/api/articles/${article._id}/attachments`)
        .auth('localAdmin', 'localAdmin')
        .field('_locale', 'enUS')
  ...
  describe('Query Language', () => {
    ...
    it('should support $or for match any rules in array', async () => {
      let response
      response = await runQuery({hobbies: {$or: ['sports', 'drugs']}})
      response.body.should.have.length(1)

      response = await runQuery({hobbies: {$or: ['sports', 'music']}})
      response.body.should.have.length(1)

      response = await runQuery({hobbies: {$or: ['drungs', 'alcohol']}})
      response.body.should.have.length(0)
    })

    it('should support $and for match all rules in object', async () => {
      let response
      response = await runQuery({vitals: {$and: [{weight: '100kg'}, {height: '5ft'}]}})
      response.body.should.have.length(0)

      response = await runQuery({vitals: {$and: [{weight: '100kg'}, {height: '6ft'}]}})
      response.body.should.have.length(1)
    })

    it('should support $or for match any rules in object', async () => {
      let response
      response = await runQuery({vitals: {$or: [{weight: '100kg'}, {height: '5ft'}]}})
      response.body.should.have.length(1)

      response = await runQuery({vitals: {$or: [{weight: '100kg'}, {height: '6ft'}]}})
      response.body.should.have.length(1)

      response = await runQuery({vitals: {$or: [{weight: '90kg'}, {height: '7ft'}]}})
      response.body.should.have.length(0)
    })
