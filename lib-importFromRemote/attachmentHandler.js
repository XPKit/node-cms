...
async function downloadBinaries(importer) {
  ...
  return await pAll(
    _.map(importer.binaryMap, (data, resource) => {
      return async () => {
            ...
            _.each(attachments, (attachment) => {
              funcs2.push(async () => {
                ...
                const filePath = path.resolve(
                  path.join('.', 'cached', resource, _.get(item, _.first(uniqueKeys)), field, filename),
                )
                try {
...
async function loadAttachmentMapData(importer) {
  ...
  const list = fs.readdirSync(seedDataPath)
  _.each(list, (name) => {
    ...
    _.each(keyList, (key) => {
      ...
      _.each(fieldList, (field) => {
        ...
        _.each(fileList, (file) => {
          const filePath = path.join(seedDataPath, name, key, field, file)
          if (!fs.lstatSync(filePath).isFile()) {
            return
          }
          let attachment
          importer.attachmentMap[name].push(
            (attachment = {
              resource: name,
              [uniqueKey]: key,
              file: filePath,
          ...
          const cmsObject = _.find(importer.cmsRecordsMap[attachment.resource], { [uniqueKey]: attachment[uniqueKey] })
          if (cmsObject) {
  ...
  await pAll(
    _.map(_.flatten(_.values(importer.attachmentMap)), (att) => {
      return async () => {
  ...
  return pAll(
    _.map(importer.config.resources, (resource) => {
      return async () => {
        ...
        const cmsRecordList = importer.cmsRecordsMap[resource]
        const list = _.filter(importer.attachmentMap[resource], (item) => {
          ...
          _.map(list, (item) => {
            return async () => {
              if (!createdRecordsMap || _.find(createdRecordsMap[resource], { _id: item._id })) {
                const foundAttachmentData = _.find(importer.binaryMap[resource], (binary) =>
                  _.isEqual(_.pick(binary, uniqueKeys), _.pick(item, uniqueKeys)),
                )
  ...
  return await pAll(
    _.map(reversedResources, (resource) => {
      return async () => {
        ...
        let cmsAttachmentList = _.compact(
          _.flatten(
            _.map(importer.cmsRecordsMap[resource], (item) =>
              _.map(item._attachments, (attach) => _.extend({ recordId: item._id }, attach)),
            ),
          ),
        )
        cmsAttachmentList = _.filter(cmsAttachmentList, (item) => {
          return _.isUndefined(
            _.find(list, _.extend({ _id: item.recordId }, _.pick(item, ['_name', '_md5sum', '_filename']))),
          )
        })
        await pAll(
          _.map(cmsAttachmentList, (item) => {
            return async () => {
              if (!createdRecordsMap || _.find(createdRecordsMap[resource], { _id: item._id })) {
