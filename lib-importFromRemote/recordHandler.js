...
async function createDummyRecords(importer) {
  ...
  await pAll(_.map(importer.config.resources, async (resource) => {
    return async () => {
      if (!importer.data[resource]) {
        return false
      }
      const remoteUniqueKeys = importer.remoteApi(resource).getUniqueKeys()
      let schema = importer.remoteSchemaMap[resource].schema
      if (remoteUniqueKeys.length === 0) {
        const firstField = _.get(schema, '[0]')
        if (firstField && !firstField.source) {
          importer.logger.warn(`No unique keys for resource ${resource}, using first field "${firstField.field}" as unique key.`)
      ...
      const createdRecords = await pAll(_.map(importer.originalRemoteRecords[resource], (item) => {
        return async () => {
...
async function deleteUnusedRecords(importer) {
  ...
  return await pAll(_.map(reversedResources, (resource) => {
    return async () => {
      ...
      let existingKeys = _.map(importer.data[resource], item => _.pick(item, uniqueKeys))
...
function getLocalAndRemote(importer, resource, uniqueKeys, record, createdRecordsMap) {
  ...
  if (!remoteRecord) {
    importer.logger.warn(`No matching remote record found for local record in resource ${resource}:`, _.pick(record, uniqueKeys))
    return {}
  }
...
async function processUpdatedRecords(importer, resource, schema, uniqueKeys, data, createdRecordsMap) {
  ...
  const upatedRecords = await pAll(_.map(data, (localRecord) => {
    return async () => {
      const { remoteRecord, createdRecord } = getLocalAndRemote(importer, resource, uniqueKeys, localRecord, createdRecordsMap)
        ...
        if (field.localised) {
          ...
          if (!_.get(field, 'source', false)) {
            ...
            if (field.input === 'multiselect') {
              ...
              const localIds = _.compact(_.map(remoteIds, (remoteRelatedId) => _.get(importer.remoteToLocalIdMap, `${field.source}.${remoteRelatedId}`, null)))
              toUpdate[field.field] = localIds
            } else {
              toUpdate[field.field] = convertKeyToId(field, _.get(remoteRecord, field.field), importer.remoteToLocalIdMap, importer.originalRemoteRecords)
            }
          }
        }
      })
      ...
      try {
        ...
        importer.logger.error(`Failed to update record ${createdRecord._id} in resource ${resource}:`, _.get(error, 'response.body', error))
        return false
      }
    }
  }), { concurrency: 10 })
...
async function updateCmsRecords(importer, createdRecordsMap) {
  ...
  await pAll(_.map(importer.config.resources, (resource) => {
    return async () => {
      if (!importer.data[resource]) {
        return
      }
...
function checkForSchemaErrors(importer, resource, schema, data) {
  _.each(data, item => {
    _.each(schema, field => {
      ...
      if (_.isNil(v) || _.isEmpty(v)) {
        ...
        const newValues = _.map(v, val => convertKeyToId(field, val, importer.remoteToLocalIdMap, importer.originalRemoteRecords))
...
async function cacheCmsRecords(importer) {
  const relationResources = _.uniq(_.compact(_.flattenDeep(_.map(importer.config.resources, resource => _.map(importer.remoteSchemaMap[resource].schema, 'source')))))
