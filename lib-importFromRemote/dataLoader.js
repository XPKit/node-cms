...
async function downloadDataFromResourceList(importer, resourcesToSync) {
  ...
  await pAll(
    _.map(resourcesToSync, (resource) => {
      return async () => {
  ...
  return await pAll(
    _.map(resourceList, (resource) => {
      return async () => {
        ...
        const schema = _.filter(
          _.get(importer.remoteSchemaMap[resource], 'schema', []),
          (field) => !_.includes(['file', 'image'], field.input),
        )
              ...
              _.each(matches, (match) => {
                ...
                if (attachmentField.localised && locales.length > 0) {
                  _.each(locales, (locale) => {
                    binaryObj[`${match.path}.${locale}`] = getAttachments(
                      newAttachment,
                      `${match.path}.${locale}`,
                      importer.config,
                    )
